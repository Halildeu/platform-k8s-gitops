...
| 2026-05-14 | ~20:00 | Adım 11.2b-2 | PR [platform-backend#185](https://github.com/Halildeu/platform-backend/pull/185) açıldı; Codex iter-25 **PARTIAL** (CI workflow blocker: `-Dtest='SqlBuilderMssqlIntegrationTest'` only ran one IT, WorkcubeQueryAdapterIT not picked up) absorb → iter-26 **AGREE** `ready_to_merge: true` (canonical-only scope; yearly+composite+`{tenantSetupProcessCatRelation}` 11.2c'ye defer) | ⏳ CI yeşillenmesi bekleniyor → REST merge | `019e258f` iter-25 PARTIAL + iter-26 AGREE |
