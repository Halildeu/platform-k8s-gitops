...
- **Status**: 🟢 source/desired-state %100 — PR-1 + PR-2a + PR-2b1 + PR-2b2a + PR-2b2b/2b3 + PR-3a + PR-3b + PR-3c + PR-4a MERGED; PR-4 live smoke pending (operator gate — agent source işi YOK)
...
- **PR slicing** (Codex `019e2a5c` AGREE):
  ...
  - **PR-4a**: schema-service target contract emission — new `GET /api/v1/schema/reporting-contract` endpoint (`ReportingContractSnapshot`/`Table`/`Column` DTOs with snake_case `@JsonNaming`, `SchemaReportingAllowlist` 40-table V1 mirror of report-service `ReportingAllowlist`, `ReportingContractService` server-side allowlist filter + deterministic `(schema,name)`/ordinal sort + `dataType→type` mapping, internal-key auth, 404 fail-closed on empty allowlist intersection) + etl-worker default snapshot path migration (`DEFAULT_SNAPSHOT_PATH` → `/reporting-contract`, new `SCHEMA_SERVICE_SNAPSHOT_PATH` env). Legacy `/api/v1/schema/snapshot` untouched (frontend + report-service unaffected). — [platform-backend#220](https://github.com/Halildeu/platform-backend/pull/220) ✅ MERGED (Codex `019e2d64` Opt-B′ AGREE; 66 schema-service + 268 etl-worker tests)
  - **PR-4**: live smoke against testai schema-service + reports DB writes — pending (operator gate: DBA `etl_snapshot_runs` table migration + Vault seed + schema-service deploy + runbook `docs/etl-worker-testai-smoke-runbook.md`). **Agent source işi YOK** — tamamen operator action.
- **DoD** (Adım 12 overall):
