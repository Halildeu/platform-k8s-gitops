...
Service: vault-cluster AppRole auth (ESO runtime path)
Status: Canonical (gitops authoritative)
Owner: @team/platform

> **Provenance:** platform-ssot `docs/04-operations/RUNBOOKS/RB-vault-approle-setup-stage.md` → migrated as-is (Faz 18.4 Phase 1). ESO ClusterSecretStore AppRole auth setup için referans, ADR-0002 §3.6 env-split policy ile hizalı.
