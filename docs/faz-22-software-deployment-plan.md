...
| D29/D30 runtime discipline | PARTIAL | test digest/runtime evidence strong; prod endpoint-admin workload PR #1242 still D29/drift gate failure. Verified 2026-06-05: prod drift gate fails because `endpoint-admin-service` is rendered while `docs/operations/services.yaml` still says `prod: deferred`; D29 gate fails because prod digest `sha256:f9c1e3aa1d5adea353422030c0a91dd170c5c162ec373c31adde0b618976e602` has no `release-candidates/` D29-GREEN ledger entry. |
| Signed agent distribution / self-update | OPEN | AG-029 signed manifest, hash verification, rollback guard and service watchdog not implemented |
| Rollout controls | OPEN | BE-026/BE-027/BE-028/BE-029 ring/window/throttle/bundle controls not implemented |
...
| Prod endpoint-admin presence | OPEN | #1241 ESO PR green/mergeable; #1242 prod workload PR has two concrete blockers: (1) `services.yaml` prod state must be advanced from deferred to enabled only when prod activation is intentionally approved; (2) endpoint-admin prod digest `sha256:f9c1e3aa...` must first be test-promoted and ledgered with D29 Up + Functional + Zanzibar evidence. |
...
- Testai kanıtı 2026-06-04 itibarıyla vardır: AG-028 multi-PR chain issue
  ...
  `ABSENT_VERIFIED`.
- Prod bu fazda hâlâ dark kalır; prod endpoint-admin PR'ları #1241/#1242 ayrı
  D29/D30 gate ister.
