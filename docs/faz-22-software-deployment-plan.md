...
| Signed agent distribution / self-update | OPEN / draft source | AG-029 draft PR #59 CI GREEN (6/6) adds staging primitives and capability false-advertising guard; signed release catalog, real Windows self-update smoke, rollback/watchdog acceptance and provider-distinct review remain pending |
| Rollout controls | OPEN / draft source stack | BE-026 PR #478, BE-027 PR #480, BE-028 PR #482, BE-029 PR #484 and BE-030 dependent DB guard PR #487 are draft/CI-green; merge, image/digest rollout, testai acceptance and provider-distinct review remain pending |
...
| **AG-029** | `platform-agent` | Signed agent self-update | **DRAFT PR + CI GREEN (PR #59, head `8a1870c`)** | Staging primitives + parseable-version capability gate source-ready; signed manifest/release catalog, hash + version policy live smoke, rollback/watchdog and provider-distinct review pending |
...
- 2026-06-06: platform-agent PR #59 is draft/CI-green (6/6) at `8a1870c`
  and closes a source-side false-advertising gap by advertising UPDATE_AGENT
  only when the local agent version is parseable. Acceptance still requires a
  dedicated backend release/catalog path, signed manifest evidence, Windows
  live self-update smoke and provider-distinct review or explicit owner
  exception.

### 22.5.8 Controlled Rollout Policies

- `BE-026`, `BE-027`, `BE-028` ve `BE-029`.
- 2026-06-06: BE-026 PR #478, BE-027 PR #480, BE-028 PR #482 and BE-029 PR
  #484 are draft/CI-green; BE-030 dependent UPDATE_AGENT DB guard PR #487 is
  also draft/CI-green on top of the bundle branch. Review/merge, image/digest
  rollout and testai acceptance remain pending before this stack can be treated
  as accepted.
...
20. `platform-agent`: `AG-029` signed update. **DRAFT PR + CI GREEN** — PR #59 head `8a1870c`; staging/source guard only, live Windows self-update smoke + signed release/catalog path pending.
21. `platform-backend`: `BE-026` + `BE-027` + `BE-028` + `BE-029` rollout ring/window/throttle/bundle controls. **DRAFT PR STACK + CI GREEN** — #478/#480/#482/#484 plus BE-030 DB guard #487; merge/testai/provider-review pending.
