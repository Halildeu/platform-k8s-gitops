...
| **M4 Signed WiX MSI** | Authenticode signed MSI, fixed UpgradeCode, service install/upgrade/uninstall, EDR allowlist doc | `platform-agent` CI + operator signing | **Needs Verify / #1680 version-floor guard added** | #115 lab MSI lifecycle smoke passed on snapshot-backed local Parallels Windows 11 (`0.1.2` install/repair -> `0.1.3` upgrade -> uninstall). Trusted MSI run `27741877258` produced `EndpointAgent-0.2.9-signed.msi` SHA256 `5cab18d460720e5bf89ddf0038f5b1c4d5ae04afc031dda0dc15d9810c969571`, `production=true`, `trusted-internal-ca`, and `timestamped=true`; the MSI and manifest are durable `v0.2.9` release assets. Current public agent artifact is now `v0.2.10` (`EndpointAgent.zip` SHA256 `fa72f278b81497bf2480ea312c7d13cff410372bfcef6ddca23dc3e50a1f292e`) and the `v0.2.10` release has no signed MSI asset. Therefore #1680 evidence must run collectors with `ExpectedMinimumAgentVersion=0.2.10`; a `v0.2.9` signed MSI run is a downgrade/regression lab unless a temporary exception is explicitly recorded, and it cannot close current runtime acceptance. |
| **M5 Same-day selected-device pilot** | Pilot OU'ya GPO Software Installation for domain-gpo devices; local Parallels/control + denetim evidence lanes; T0/T+15/T+60 collector monitoring | Operator + IT + gitops evidence | **aynı gün** (no-24h owner direction) | Board #1377 accepted the earlier same-day selected-device scope; #1609 narrows active productization to max 2 devices and #1643 accepts AgentPC2 as the third product-channel device. Signed MSI/GPO rollout acceptance is split to #1680 before any broader wave; #1680 now requires a single selected method (`gpo-msi` or `one-command-bootstrap`), constrained pilot targeting, per-device collector evidence with the current version floor, and one rollback drill. |
