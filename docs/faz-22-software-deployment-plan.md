...
| **M5 Same-day selected-device pilot** | Pilot OU'ya GPO Software Installation for domain-gpo devices; local Parallels/control + denetim evidence lanes; T0/T+15/T+60 collector monitoring | Operator + IT + gitops evidence | **aynı gün** (no-24h owner direction) | Board #1377 accepted the earlier same-day selected-device scope; #1609 now narrows active productization to max 2 devices. Primary acceptance pair: `SRB-AIDENETIMPC` + `AgentPC2`; reserve/lab: `AgentPC1` + local Parallels Windows. Tokenless enroll + heartbeat + inventory/result-submit, no manual token/ZIP for domain-gpo devices, 1 cihaz rollback + reinstall drill, post-pilot artifact states `same_day_smoke=true` and `soak_hours=0`. |
| **M5A Domain Ops Broker gate** | Delegated Windows service for EndpointTest OU / EndpointAgentPilotComputers / Endpoint Agent GPO mutation; maker-checker + TTL + audit | `platform-k8s-gitops` plan + backend/ops implementation | **P0 design gate** | SSH/RDP clipboard is not the durable AD/GPO mutation channel. Broker design is documented in `docs/faz-22-domain-ops-broker-plan.md`; first runtime slice must prove read-only state + bounded pilot group/GPO mutation + rollback audit. |
| **M5B Remote-ops MVP safety gate** | Endpoint-side outbound mTLS operatorless diagnostics/remediation; default-off, TTL, bounded command allowlist, no raw shell | `platform-agent` + backend broker surface | **P0 implementation gate** | Remote-ops cannot unblock AgentPC2/AD/GPO operations until product code enforces default-off, TTL, allowlist-only commands, no arbitrary shell, and audit/result/log evidence. Temporary reverse SSH is lab-only. |
| **M6 50-PC Wave** | Ring/tag rollout, concurrency/maintenance window discipline, alerting | Operator + backend rollout controls | **GATE CLOSED by #1609** | Do not start a 50-PC denominator while only 2 active pilot devices are available. Reopen only after Denetim PC + AgentPC2 acceptance, remote-ops MVP safety gate, signed MSI/GPO smoke, rollback/reinstall proof, and explicit owner acceptance. |
...
pilot cihaz sağlayabildiği için, sektör-standardına yakın güvenli yol
şudur: Denetim PC + AgentPC2 primary pair olarak seçilir; AgentPC1 reserve,
local Parallels Windows kırma/onarım laboratuvarı olarak kalır. AgentPC2
domain-joined ve GPO/cert yolu çalışıyorsa signed MSI + GPO ana kanal olur;
değilse tek komut bootstrap geçici pilot kanalıdır. AD/GPO mutation için
kalıcı model SSH değil, `docs/faz-22-domain-ops-broker-plan.md` içindeki
delegated Domain Ops Broker'dır.
