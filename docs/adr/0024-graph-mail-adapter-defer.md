...
**2026-09-11 (D7c — backend GraphMailAdapter PROD cutover, adapter-LIVE)** — Owner kararı: `ai@acik.com` SMTP parolası 2026-09-04'te süresi doldu (#3547, `535 5.7.139`) ve **yenilenmeyecek**; prod notification-orchestrator mail yolu da Graph app-only'ye geçti (gitops#3692 `87451a6e`, owner onayı PR yorumunda). Prod Vault `kv/platform/notification-orchestrator` v10 `graph_*` (in-band kopya, `kv/platform/graph` ile eşitlik doğrulandı); prod ESO 3 remoteRef re-enabled → `SecretSynced`, Secret'ta 3 GRAPH anahtarı; ConfigMap `NOTIFY_ADAPTERS_GRAPH_ENABLED=true` + sender `ai@acik.com` + saveToSentItems; selektif `kubectl apply` (prod ArgoCD app auto-sync değil) + `rollout restart` → podlar `55c7994785-45zd2/-dwz66` Running, log `GraphMailAdapter initialized senderMailbox=ai@acik.com` + `GraphTokenService initialized` (tenant/client id prefixes match the seeded identity); eski SMTP podu sonlandı (RS 0). SMTP config bayrak-geri-alma için yerinde (ama SMTP parolası ölü → gerçek geri alma = parola rotasyonu). **Prod functional smoke PROVEN 2026-09-11 20:27 UTC** (owner "kur ve koş"): BL-011 canary yolu e-posta için kuruldu — şablon `graph-smoke-prod-mail-v1` (v1 tr-TR, external_allowed=false), abone `graph-smoke-prod-canary-001` (org default, ai@acik.com verified), OpenFGA tuple'ları `notification_topic:marketing.campaign#can_receive@subscriber:graph-smoke-prod-canary-001` + `template:graph-smoke-prod-mail-v1#topic@notification_topic:marketing.campaign` (model `01KSFFK9K3V43DD211Z79K3FYA`, check allowed); persona `notify-canary-org-prod-default` + `frontend` client `openid notify-canary` → `POST /api/v1/notify/intents` (`channels:["email"]`) → **202** `graph-smoke-prod-20260911-202702` → intent COMPLETED, delivery DELIVERED `provider_msg_id <74df92b5-…@notification-orchestrator-graph>`, pod log `graph mail accepted … status=202`, `ai@acik.com` kutusunda 20:27:08Z/20:27:10Z Sent+Inbox kayıtları. #3547 closed. D1 "prod SMTP canonical" ifadesi bu tarihten itibaren geçersiz: prod da Graph.
