...
- **22.2.A non-domain primary**: ~80% (strong evidence — PR #1021 BE-011 + AG-013 + #1032 BE-017 fixture + PR #13 CI automation source + **platform-agent PR #17 (`91ef533d`) AG-021/AG-022 identity source-foundation MERGED 2026-05-26** with `WORKGROUP`/`LOCAL` HALILKOOLUB735 read-only evidence, redact pass clean; **eksik**: self-hosted CI run, 2+ standalone/BYOD device, 24-72h soak, signed distribution / support / KVKK boundary. *Identity classification source path artık DONE (AG-021/AG-022 source-foundation); field acceptance — multi-device classification + soak + signed binary — operator-bound gates altında pending; #1044 PASS DEĞİL, #1037 unblocked DEĞİL.*)
