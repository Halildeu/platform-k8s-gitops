...
|---|---|---|
| **Backend source** | **Plan C H1+H2 MERGED 2026-05-22**: `platform-backend/origin/main` artık endpoint-admin-service tree + api-gateway routes/permitAll/test refactor taşıyor. H1 PR #288 mergeCommit `8e2589c1`; H2 PR #291 mergeCommit `161296cf`. Final main D30 immutable images produced. **C.5.persona Live JWT 6/6 matrix VERIFIED LIVE** post-#961 ConfigMap fix `a29fd55f`. | BE-011 agent live integration + BE-014 tamper/offline audit event model + D35-EA audit variants smoke. |
| **Backend live** | Test cluster digest pin `sha256:b2250b7a274ec8...` (endpoint-admin, gitops PR #956) + api-gateway `sha256:84500b5ebe162b...` (gitops PR #957) + **#961 ConfigMap fix mergeCommit `a29fd55f`** (SPRING_PROFILES_ACTIVE=k8s + gateway routes 22/23/24). **D29-EA Secured VERIFIED LIVE**: synthetic OpenFGA 5/5 (#959) + Live JWT 6/6 persona matrix (admin allow GET/POST, viewer can_view allow, viewer can_manage DENY 403, no-token 401, invalid 401) + DB audit row (`endpoint_enrollments` PENDING + `endpoint_audit_events` action=CREATE_ENROLLMENT) + OpenFGA pod authz log deny path observable. | BE-013 maintenance-token full live lifecycle (issue/validate/revoke/expiry); BE-011 agent live integration; D35-EA audit variants smoke (BE-014 tamper/offline). |
