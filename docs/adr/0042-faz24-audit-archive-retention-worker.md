...
Değişmezlik **üç katman**: (i) DB append-only trigger + BE-016 hash-chain (kaynak), (ii) S3 **Object Lock COMPLIANCE / WORM** per-object 7yr (arşiv), (iii) manifest+object SHA-256 digest (içerik bütünlüğü).
...
3. **State tabloları** (worker'a ait, ayrı Flyway):
   - `audit_archive_cursor` (singleton): `last_archived_seq bigint`, `updated_at`.
   - `audit_archive_ledger` (arşivlenen her obje için bir satır): `object_key`, `chain_scope`, `min_seq`, `max_seq`, `row_count`, `min_event_timestamp`, `max_event_timestamp`, `entry_hash_alg`, `entry_hash_version`, `object_sha256`, `manifest_sha256`, `retention_until`, `worker_image_digest`, `verify_status`, `created_at`, ve anchor temsili — **GLOBAL** → `first_prev_hash` + `last_entry_hash`; **PER_TENANT** → `tenant_anchors` (JSONB, §D4.6 temsili).
4. **Verify-before-archive (fail-closed)** — **`CHAIN_SCOPE ∈ {GLOBAL, PER_TENANT}`** parametresine göre:
   ...
   - **HARD GATE (C-slice)**: `CHAIN_SCOPE` C başında **consumer/BE-016 kodundan teyit edilir** (sentetik varsayım YASAK). Teyit edilemezse **C başlamaz** ve bu ADR teyit edilen scope ile **amend** edilir. V1 worker tek modda derlenir; runtime'da scope flip etmez.
...
7. **Yazım sırası (cursor YALNIZ hepsinden sonra ilerler)** — object key `min_seq–max_seq`'ten deterministik türetilir; **idempotency re-put ile DEĞİL, HEAD-reuse ile** (Object Lock + versioning'de re-put yeni version yaratır; overwrite YASAK): önce `HEAD object` — (a) **varsa** → ledger'daki `object_sha256` + retention ile doğrula; eşleşirse **PutObject atlanır**, ledger/cursor tutarlılığı upsert ile sağlanır, cursor ilerler; eşleşmezse fail-closed alert (beklenmeyen/değişmiş içerik); (b) **yoksa** → PutObject(NDJSON.gz, retention=COMPLIANCE retain-until=now+7yr) → HEAD verify (size + etag + retention header) → PutObject(manifest.json) → HEAD verify → `audit_archive_ledger` insert → `audit_archive_cursor` advance. Bucket policy + worker cred (§D5) overwrite/delete'i **reddeder**; idempotency HEAD-reuse'a dayanır. *(C-impl notu: HEAD-reuse doğrulamasında ETag tek başına güvenilir değildir — `object_sha256` bağlama PutObject'te S3 checksum header `x-amz-checksum-sha256` veya immutable user-metadata ile yapılır; salt ETag karşılaştırması YASAK.)*
...
- **D29 (A — bu oturum)**: MinIO Up; `audit-archive` bucket object-lock + versioning + COMPLIANCE 7yr default retention configured; in-cluster pod'dan **put/get/head OK**; **overwrite/delete DENIED**; retention header doğru; ESO/Vault path + NetPol yalnız gerekli trafiğe izin.
- **C worker**: Testcontainers (MinIO + PG); `CHAIN_SCOPE` consumer'dan teyit (D4.4 hard gate); no-gap contiguous-prefix cursor (atlanan satır yok); idempotent re-run (HEAD-reuse, overwrite denied); verify-before-archive fail-closed (kırık chain → no-archive + alert); manifest digest round-trip; least-privilege cred (delete/overwrite denied).
