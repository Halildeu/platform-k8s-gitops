...
> **STATUS (2026-05-17 v2) — B1a-dedup browser reconcile**: Gerçek-tarayıcı ölçümü (testai `/access/roles` fresh reload, real shell, non-chart MFE route) B1a-dedup'ı **yeniden çerçeveledi**. (a) Yukarıdaki "chart 7× dedup ~55 MB" **DE-SCOPED** — chart stack lazy code-split; non-chart route heavy chart chunk indirmiyor (V3-B2 pattern: headline figür browser'da reprodüksiyon vermedi); kalan = düşük öncelikli build/CDN hygiene. (b) **GERÇEK bulgu**: tek route'ta **5× `@mfe/design-system` loadShare chunk** (~1.75 MB transfer / ~6.45 MB decoded each) — route JS transfer'inin ~%91'i; MF singleton dedupe ETMİYORDU (6 remote DS'i `singleton()` deklare etmiş, `hostOnly()` değil). **Fix**: platform-web PR #570 (`fcb919dd`, Codex `019e3333` AGREE) — 6 remote `@mfe/design-system` → `hostOnly()`; 6/6 build clean, loadShare chunk kalktı. `@mfe/shared-http`/`@mfe/i18n-dicts` aynı drift (ayrı PR).
>
> **⚠ DÜZELTME (2026-05-17 v3)**: Yukarıdaki "Fix: PR #570" **GEÇERSİZ** — PR #570 **REVERT EDİLDİ** (platform-web PR #571, `95e6fb5d`). `hostOnly`/`import:false` `@mfe/design-system` için (workspace paketi, derlenmiş `dist/` yok) `build:ubuntu:single-domain` post-merge image build'ini `UNLOADABLE_DEPENDENCY` ile kırdı; PR CI'ı (`build:raw`) bunu görmedi. **DS-dedup bulgusu geçerli** (browser-verified 5× loadShare); fix-redo Codex `019e3333` 2-adımlı plan: (1) `packages/design-system` temiz build edilebilir, (2) `build-single-domain.mjs` MFE build'lerinden önce design-system'i `dist/`'e derlesin, (3) sonra DS `hostOnly()` + `build:ubuntu:single-domain`'i PR check'e ekle. Detay: `session-handoff-2026-05-17-v3-b1a-dedup.md` üst CORRECTION bloğu.
