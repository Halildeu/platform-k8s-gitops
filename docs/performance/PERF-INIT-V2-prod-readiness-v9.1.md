...
| **PR-V2.1-ABM-1** | 4-canary reproducibility soak | Measurement **artifact + runbook** (CI workflow DEĞİL — Codex tur-2 absorb: lab vs live karışmasın); **min 3 doğal cron fire/cluster sustained PASS** pre-prod acceptance (Codex `019e2a4f` Q1 B-prime reclassification: pre-prod aktif kullanıcı yokken 24-72h synthetic wait yeni signal üretmez; real-traffic 24-72h post-cutover scope — Faz G rollback-window + RUM telemetry); budget delta variance < %5 |
