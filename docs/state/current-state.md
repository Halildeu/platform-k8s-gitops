...
> **Status as of**: 2026-04-22 ~09:35 UTC+3 (Session 13 live edge + login callback closure refresh)
...
|---|---:|---|---|---|---|---|
| **test-k8s** | **89** | testai.acik.com public edge zinciri yeniden sağlıklı: OIDC discovery + canonical frontend render canlı, public `/` `200`, login callback route `/login` `200`, no-token `/variants` `401`. Host nginx `testai /api` masking drift'i kapatıldı, ingress-nginx → frontend yolu `80/TCP` NetworkPolicy açığı giderilince toparlandı ve frontend nginx SPA fallback'i `/login` gibi route'ları yeniden `index.html`'e düşürüyor. Authenticated allow ise bu refresh'te yeniden mint edilmedi; 2026-04-21'de görülen `testuser` `/variants` `500` + gateway `serban` JWKS drift'i hâlâ açık risk olarak tutuluyor | 2026-04-22: `curl https://testai.acik.com/realms/platform-test/.well-known/openid-configuration` → `200`; `curl https://testai.acik.com/` → `200`; `curl https://testai.acik.com/login` → `200`; no-token `/variants` → `401`; ingress-nginx-controller pod içinden `wget http://10.44.3.226:80/login` → `200`. 2026-04-21 önceki blocker: password-grant token ile `/variants` → `500`; gateway log: `/realms/serban/protocol/openid-connect/certs` + `No suitable decoder accepted the token` | 2026-04-22 | Codex | Taze authenticated token mint + `/variants` sonucunu `2xx/403/4xx` seviyesine indir + 24h soak |
...
**Weighted operational continuity**: `~%74` (test public edge, frontend parity ve login callback toparlandı; authenticated allow, prod GitOps/ESO ve DR hâlâ açık)
...
| Gateway JWT realm drift | Test authn/authz zinciri `platform-test` realm ile kapanmalı | 2026-04-21 session'ında authenticated `testuser` tokenı ile `/variants` `500`; `api-gateway` log'u `GET /realms/serban/protocol/openid-connect/certs` çağırıp `404` sonrası `No suitable decoder` üretiyordu. 2026-04-22 edge recovery sonrası authenticated token yeniden mint edilmedi; blocker kapanmış sayılmıyor | App/Ops | Faz 11 | HIGH |
