...
## Live Delta — Faz 22.5 standard PC install productization chain (2026-06-08 16:05 Istanbul / 13:05Z UTC)
...
| Agent installer/package | `platform-agent` PR #102 merged `cba5ecee`; PR #103 merged `87fa38b9`; PR #105 merged `05774bf6`; PR #106 merged `7e92a9e4`; main workflow runs `27137185247` and `27142499833` succeeded | PS5.1-safe installer package, UTF-8 BOM packaged scripts, `EndpointAgent.zip`, `bootstrap-package.ps1`, static encoding guard and `-AutoEnroll` installer/bootstrap path are source-ready. Both agent AutoEnroll client defaults and packaged bootstrap defaults align to the deployed external route `/api/v1/endpoint-agent` |
| Canonical artifact | `https://testai.acik.com/artifacts/endpoint-agent/0.1.0-dev/EndpointAgent.zip` returned HTTP 200 with ZIP SHA256 `88af3d2539c1c0a8a0fcf9525e38238d7816d02eb2f1c3789725b771fe088cf0`; standalone `bootstrap-package.ps1` SHA256 `7ac13aad5c910a74c59862dfc7faafc3c88187c541b9b5f7af64172427335859`; internal `SHA256SUMS` matched; packaged `bootstrap-package.ps1`, `install.ps1`, `uninstall.ps1` carry BOM; public HTTPS verification confirmed the bootstrap script contains `/api/v1/endpoint-agent` and does not contain stale `/api/v1/endpoint-admin` AutoEnroll base | Manual ZIP transfer is no longer required for HMAC pilot bootstrap; hidden token prompt still remains on the HMAC fallback path |
