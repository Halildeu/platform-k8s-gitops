...
> **Status as of**: 2026-04-22 ~04:10 UTC+3 (Session 13 source-deploy rerun + live edge re-probe)
> **Verified by**: Codex + live `ssh staging-sw`
> **Source set**: Live `kubectl`, `curl`, `docker`, Prometheus API, `ssh staging-sw` outputs + repo HEAD + `Halildeu/platform-ssot` Actions run `24753798988`
...
|---|---:|---|---|---|---|---|
| **test-k8s** | **70** | Source repo stage deploy hattı yeniden yeşile döndü; stage backend/stateful zinciri host üzerinde healthy. Ancak public `testai.acik.com` yüzeyi şu an authoritative değil: OIDC discovery `200` dönse de root `504`, no-token `/variants` `503`; staging-sw üzerinde `127.0.0.1:9080` refused olduğu için k3d-test ingress path'i edge'den cevap veremiyor. Authenticated allow / JWT realm drift bug'ı bu outage altında yeniden doğrulanamadı | `Halildeu/platform-ssot` Actions run `24753798988` → `success` (`deploy-stage-host` + `Post-deploy health validation` yeşil); `docker inspect` → `platform-{pg,vault,kc}-test` + backend zinciri healthy; `curl https://testai.acik.com/realms/platform-test/.well-known/openid-configuration` → `200`; `curl https://testai.acik.com/` → `504`; `curl https://testai.acik.com/api/variant/api/v1/variants` → `503`; hostta `curl http://127.0.0.1:8080/actuator/health` → `200`, `curl http://127.0.0.1:9080/` → connect refused | 2026-04-22 | Codex | `127.0.0.1:9080` ingress/hostPort path restore + root `200` + no-token deny `401`, sonra authenticated allow `2xx/403` re-probe |
...
**Weighted operational continuity**: `~%64` (stage deploy green ve test stateful/backend zinciri healthy; ancak public test edge yeniden bozuk, prod GitOps/ESO ve DR hâlâ açık)
...
| `testai.acik.com` | staging-sw host nginx → 127.0.0.1:8082 (KC) + 127.0.0.1:9080 (k3d-test ingress) | platform-kc-test + platform-pg-test + platform-vault-test + stage backend zinciri healthy; fakat public ingress path'i şu an cevap vermiyor | OIDC `200`; root `504`; no-token `/variants` `503`; hostta `127.0.0.1:9080` → connect refused; `127.0.0.1:8080/actuator/health` → `200` |
...
| Test ingress edge path | `testai.acik.com` root + `/api/variant` trafiği staging-sw → `127.0.0.1:9080` üzerinden authoritative akmalı | `127.0.0.1:9080` refused; public edge root `504`, no-token `/variants` `503`; aynı anda lokal gateway `127.0.0.1:8080/actuator/health` `200` ve Keycloak test OIDC `200` | Ops/App | Immediate | HIGH |
