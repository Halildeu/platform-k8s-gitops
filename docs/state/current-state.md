...
**İddia sınırı**: PR-3C'nin least-privilege iddiası dar — "`deploy-prod-gitops.yml`
prod WORKFLOW path'i artık `admin@k3d-prod` kullanmıyor; workflow k8s identity
`prod-deploy-smoke` ile sınırlı". Operator'ün `~/.kube/config`'indeki
`admin@k3d-prod` PR-3C'de DOKUNULMADAN kalır (manuel/break-glass erişim) —
host/user trust boundary PR-3D'nin işi.
