...
## Live Delta — schema-service extractTables Timeout Fix: P0+P1+Q3 LIVE (9 PR Merged, 2026-05-17)
...
Sessions 65-66 fix zinciri: P0 (timeout) + P1 (split) + Q3 (hardening).

### PR Map (9 PR — hepsi cross-AI Codex AGREE, CI yeşil, normal squash, archive-tag)

| Aşama | PR | Repo | Konu |
|---|---|---|---|
| Handoff | #726 / #733 | gitops | Session 65 / 66 handoff doc (D28 5-alan) |
...
- schema-service pod image `sha-b9b40f7` / digest `sha256:bbf79dd96aea5fbd91a151e6440f491e8b6826f31ddee1c5c1af843bf24373f5` (digest job-section + CD deploy spec iki-kaynak teyitli).
- `GET /api/v1/schema/snapshot?schema=workcube_mikrolink` → **HTTP 200**, build ~50s, **1513 tablo / 26333 kolon / 1787 ilişki / 16 domain**.
...
- 8 B1 envanter canlı: foreignKeys 10 · uniqueConstraints 3 · defaultConstraints 607 · indexes 1834 · objects 1524 · databaseOptions ✓ · checkConstraints/changeData gerçek-boş.
- Q3 503-path 201 test (cause-leak guard dahil — SQL/JDBC URL response'a sızmıyor); happy-path HTTP 200 regresyon yok.
- schema-service standalone suite: **201 test, 0 fail** (191 → P0 +2 → P1 +5 → Q3 +3).
...
### Bilinen degrade (non-fatal — kod-blocker DEĞİL)

`storage` envanteri canlıda boş: `extractStorage` (`sys.dm_db_partition_stats`
DMV) `VIEW DATABASE PERFORMANCE STATE` izni istiyor (SQL error 262), MSSQL
`AlUser_App` hesabında yok. B1-6 non-fatal tasarımı gereği snapshot çökmedi,
degrade oldu. Kapanışı operatör/DBA `GRANT VIEW DATABASE PERFORMANCE STATE`
işi (access-control değişikliği — agent kapsamı dışı).
