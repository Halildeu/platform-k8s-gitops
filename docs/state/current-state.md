...
> **Status as of**: 2026-04-22 ~09:25 UTC+3 (Session 13 live edge closure refresh)
...
|---|---:|---|---|---|---|---|
| **test-k8s** | **88** | testai.acik.com public edge zinciri yeniden sağlıklı: OIDC discovery + canonical frontend render canlı, public `/` `200`, no-token `/variants` `401`. Host nginx `testai /api` masking drift'i kapatıldı ve ingress-nginx → frontend yolu `80/TCP` NetworkPolicy açığı giderilince toparlandı. Authenticated allow ise bu refresh'te yeniden mint edilmedi; 2026-04-21'de görülen `testuser` `/variants` `500` + gateway `serban` JWKS drift'i hâlâ açık risk olarak tutuluyor | 2026-04-22: `curl https://testai.acik.com/realms/platform-test/.well-known/openid-configuration` → `200`; `curl https://testai.acik.com/` → `200`; no-token `/variants` → `401`; ingress-nginx-controller pod içinden `wget http://10.44.3.206:80/` → `200`. 2026-04-21 önceki blocker: password-grant token ile `/variants` → `500`; gateway log: `/realms/serban/protocol/openid-connect/certs` + `No suitable decoder accepted the token` | 2026-04-22 | Codex | Taze authenticated token mint + `/variants` sonucunu `2xx/403/4xx` seviyesine indir + 24h soak |
...
**Weighted operational continuity**: `~%72` (test public edge ve frontend parity toparlandı; authenticated allow, prod GitOps/ESO ve DR hâlâ açık)
...
- ❌ "%99.5 migration complete" → ✅ "Weighted operational continuity ~%72"
