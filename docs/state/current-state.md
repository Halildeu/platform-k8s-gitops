...
| **prod-workload-gitops** | **75** | Session 27: ArgoCD platform-eso-prod manual sync → roleId UUID canlıya + CSS Ready=True + 8 ES SecretSynced=True. `openfga-migrate` Job **Complete 1/1 5s** (Session 26'da delete+recreate sonrası migration done version 6). `platform-prod` Application artık `OutOfSync/**Healthy**` (Degraded kaynak kapandı, 0 Degraded child resource). ArgoCD cosmetic OutOfSync v1beta1 stored serialization + ConfigMap null→Degraded aggregation kalıntı; Faz 13 rebuild ile doğal temizlenecek teknik borç. Skor 75 = "kanıtlı ama cutover-ready değil" (Codex scale); 90+ GitOps gate için RespectIgnoreDifferences veya rebuild | `kubectl get clustersecretstore vault-platform-gitops -o jsonpath='{.status.conditions[0].status}'` → `True`; 8 ES `True`; `kubectl -n platform-prod get job openfga-migrate` → `Complete 1/1 5s` pod logs `migration done, current version: 6`; `kubectl -n argocd get applications.argoproj.io -o wide` → `platform-prod OutOfSync/Healthy`, `platform-eso-prod OutOfSync/Healthy` | 2026-04-24 | Claude | ArgoCD cosmetic (Faz 13 rebuild) + RespectIgnoreDifferences syncOption (opsiyonel) |
