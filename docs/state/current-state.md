...
- Workload-mutate yüzeyi (patch/exec/scale, platform-prod portforward) kapalı; runner identity yalnız port-forward + read.

**kubectl 1.36 `auth can-i` slash-syntax bug**: `auth can-i create pods/portforward`
(slash formu) subresource'u yanlış değerlendirip false-`no` döndürür. Doğru form
