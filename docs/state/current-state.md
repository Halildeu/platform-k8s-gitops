...
| Live artifact-host | 🟡 STILL v0.2.5 AFTER MERGE POLL | Post-merge live poll at 2026-06-16T12:38-12:39Z still returned `release_tag=v0.2.5` from `https://testai.acik.com/artifacts/endpoint-agent/current/release-manifest.json`. Local Codex context has no `k3d-test` kube context and direct staging SSH failed with `Permission denied (publickey,password)`, so pod imageID/ArgoCD sync could not be verified from this environment. Do not claim v0.2.7 live until `/current/release-manifest.json`, `/current/EndpointAgent.zip.sha256`, and artifact-host pod imageID match the v0.2.7 digest. |
