...
## Live Delta — Faz 22.5 Software Deployment plan added (2026-05-27)

Faz 22 Endpoint-Enes hattına **software deployment quick-win** planı eklendi.
Bu delta yalnız dokümantasyon/roadmap seviyesidir; yeni runtime kabiliyeti iddia
etmez.

| Katman | Durum |
|---|---|
| Plan | `docs/faz-22-software-deployment-plan.md` eklendi |
| Runbook | `docs/runbooks/RB-faz22-software-deployment-winget.md` eklendi |
| Governance | ADR-0012-EA içine 22.5 WinGet + Approved Software Catalog sınırı eklendi |
| Board | platform-k8s-gitops#1083 tracking issue |
| Runtime | Yeni install/uninstall kabiliyeti yok; source PR'ları ilgili repolarda ayrıca açılacak |
...
`AG-033` disk/RAM/uptime health snapshot, `BE-020` Approved Software Catalog
API, `AG-027` approved install command, `BE-021` result/detection/audit,
`WEB-011` inventory/posture view, `WEB-012` approved install UI, `AG-028`
