...
#
# Exit codes:
#   0 — stable_30d=1 AND continuous 30d ready: M8 DoD blocker #1 met
#   1 — stable_30d=1 but not yet continuous 30 days: window in progress
#   2 — stable_30d=0: observation NOT stable; M8 blocker not met
#   3 — Prometheus unreachable / metric absent / observation_present=0
#   4 — Usage error
#
...
OBS_PRESENT=$(query 'notify:m7_v1:observation_present:30d{namespace="platform-prod"}')
STABLE_NOW=$(query 'notify:m7_v1:stable_30d{namespace="platform-prod"}')
CONTINUOUS_READY=$(query 'min_over_time(notify:m7_v1:stable_30d{namespace="platform-prod"}[30d])')

# Verdict logic — match the boolean composition in the recording rule.
verdict() {
  if [[ "$OBS_PRESENT" == "null" || "$OBS_PRESENT" == "0" ]]; then
    echo "OBSERVATION_ABSENT"
    return
  fi
  if [[ "$STABLE_NOW" == "1" && "$CONTINUOUS_READY" == "1" ]]; then
    echo "M8_DOD_BLOCKER_MET"
  elif [[ "$STABLE_NOW" == "1" ]]; then
    echo "STABLE_BUT_WINDOW_IN_PROGRESS"
...
cat >"$OUT" <<EOF
{
  "schema_version": "m7-v1-30day-stable-evidence/v1",
  ...
  "predicates": {
    ...
    "observation_present_30d": ${OBS_PRESENT}
  },
  "thresholds": {
    "dispatch_success_rate_30d_min": 0.995,
    "dlq_burn_24h_max_30d_max": 1.0,
    "dlq_burn_72h_max_30d_max": 1.0,
    "critical_alert_minutes_30d_max": 0,
    "observation_present_30d_min": 1
  },
  "stable_30d_now": ${STABLE_NOW},
  "continuous_30d_ready": ${CONTINUOUS_READY},
  "verdict": "${VERDICT}",
  "anti_pattern_guards": {
...
case "$VERDICT" in
  ...
  STABLE_BUT_WINDOW_IN_PROGRESS) exit 1 ;;
  UNSTABLE) exit 2 ;;
