...
- `platform-ai#207` meeting-ai `/ask` path'ini source seviyesinde sertleştirdi: transcript ve question redaction, residual PII fail-closed `422`, unsupported cloud backend için silent mock fallback yerine `501`. Bu yalnız G-INT source hardening'dir; gerçek pilot transcript/audio veya reviewer/operator acceptance değildir.
