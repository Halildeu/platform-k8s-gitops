...
| **G-INT** | Faithfulness + action-item precision/recall + owner/date accuracy; her summary/action citation/timecode ile bağlanır; `platform-ai#200` gate synthetic/mock kanıtı pilot acceptance yerine kullanmayı bloklar, `platform-ai#213` source-side summary prose exposure guard ekler, `platform-ai#214` due-date attribution'ı aynı kaynak cümlede kopya phrase yoksa `due_date=null` + `action_due_date` rejection olarak saklar; gerçek pilot evidence ve reviewer/operator acceptance olmadan pass üretmez |
