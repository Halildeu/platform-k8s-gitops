...
>
> **2026-06-27 desktop capture evidence addendum**: `scripts/faz24/verify_desktop_capture_evidence.py` and `docs/runbooks/RB-faz24-desktop-capture-evidence.md` package the remaining desktop mic+loopback gate as metadata-only, fail-closed evidence. A passing envelope must come from a real `platform-desktop` run, prove both `microphone` and `loopback` real-device sources, visible active capture indicator, consent capture, exact public `audio-gateway` lifecycle step order, and uploaded chunk digest matches. The verifier rejects raw audio/base64 audio, transcript text, JWT/Bearer/Authorization material, destination URLs, direct client-to-STT, direct-STT transcript, compute-plane audit, and production-readiness claims. This is not a live desktop smoke by itself and does not satisfy the external-recorder G-CAP aggregate threshold.
>
