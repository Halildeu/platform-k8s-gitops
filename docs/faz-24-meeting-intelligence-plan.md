...
- Ürün-değer hattında source-side guardrail ilerledi: G-WER/DER verifier (`platform-ai#199`), G-INT verifier (`platform-ai#200`), meeting-ai `/ask` redaction hardening (`platform-ai#207`), meeting-ai action-owner grounding (`platform-ai#208`), meeting-ai unsupported-answer withholding (`platform-ai#209`), diarization backend decision gate (`platform-ai#210`), retention readiness gate (`platform-ai#201` + MinIO runtime-evidence hardening `platform-ai#211`), Redis control-plane cleanup (`platform-ai#202`), recording/archive RED boundary (`platform-ai#203`), G-LAT/COST verifier (`platform-ai#204`), G-CAP aggregate capture gate (`scripts/faz24/verify_gcap_capture_gate_evidence.py`), G-COMP aggregate compliance gate (`scripts/faz24/verify_gcomp_compliance_gate_evidence.py`) ve G-OPS operability gate (`scripts/faz24/verify_gops_operability_gate_evidence.py`) main'de. #156 DB-cleanup runtime slice test ortamında metadata-only destruction audit ile kanıtlandı; current retention snapshot artık `findingCount=0`, `blockerCount=4` ile VERBIS + üç MinIO runtime lifecycle export blocker gösterir. Buna rağmen gerçek pilot WER/DER, gerçek pilot G-INT, pilot G-LAT/COST, live aggregate G-CAP evidence, VERBIS ve MinIO lifecycle runtime evidence, live G-COMP evidence, direct-STT e2e ve desktop mic/loopback acceptance hâlâ açık.
...
| **T-D Compliance Productization** | ADR-0030 hukuk/VERBIS acceptance, consent UI, retention/legal-hold, access matrix, audit export, on-prem install pack; #156 DB cleanup test evidence var, #185 recording/archive RED boundary ve G-COMP/G-OPS source-side verifier'ları hazır, VERBIS/MinIO/legal/live G-COMP evidence bekler | Bu pazar için farklılaşma noktası; doküman değil ürün yüzeyi olmalı | P1 | gitops + web + backend |
...
| **G-COMP** | Consent, retention, legal hold, access audit ve deletion/export policy canlı; KVKK hukuk/VERBIS boundary ADR-0030'da accepted; `platform-ai#201` retention gate mevcut durumda blocked döner ve `platform-ai#211` ile source-only MinIO lifecycle evidence'ı active kabul etmeyecek şekilde sıkılaştırıldı; `platform-ai#203` raw-audio archive'i default live path'ten çıkarır; `scripts/faz24/verify_gcomp_compliance_gate_evidence.py` yalnız redacted metadata envelope'ını kabul eder, `.github/workflows/faz24-product-gate-evidence-ingest.yml` bu kanıtı artifact'li no-mutation ingest path'e taşır. #156 DB cleanup smoke test ortamında geçti, ancak canlı VERBIS/MinIO runtime lifecycle/legal/operator evidence olmadan G-COMP pass üretmez |
