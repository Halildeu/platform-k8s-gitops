...
- `platform-ai#188` same-session `CHUNK_FORWARDED_TO_COMPUTE_PLANE` audit gate verifier PASS ile accepted/Done durumundadır. `platform-ai#182` direct-STT e2e hâlâ açık: #226 GPU timeout fix Denetim runtime'a uygulanmıştır; `platform-backend#768` artık merge/deploy edilmiş ve #2061/#2062/#2063/#2065 default-off mTLS/SNI/evidence chain hazırdır. Kalan live path: Vault/ESO credential seed, `AUDIO_GATEWAY_DIRECT_STT_ENABLED=true` flag flip, fresh `/transcribe -> DIRECT_STT_TRANSCRIPT_RESULT` evidence, same-session audit correlation ve no raw-audio persistence proof.
