...
| A4 | SMS (JetSMS primary, NetGSM secondary) | **MVP-geniş** | 23.3 | 🟢 infra LIVE / 🟡 functional canary pending (**M4 prod LIVE 2026-05-20** sha-6307428 — JetSMS primary `SmsAdapter activated: primary=jetsms` + `JetSmsDlrPollingWorker scheduling=true` + DLR DELIVERED end-to-end test cluster; multipart context routing 23.3.2 LIVE; **NetGSM secondary R1 ⏳ DEFER asset-preserved** per kullanıcı kararı 2026-05-23 — sözleşme kısa vadede yok; JetSMS-only degraded mode kabul edilen kalıcı işletim durumu; NetGsmProvider + Vault/ESO dormant. **BL-010 prod KC `org_id=default` claim setup ✅ COMPLETED 2026-05-25** (PR #1062 MERGED — `serban` realm + persona LIVE + JWT 3-way claim verified). **BL-011 prod SMS canary DEFER 2026-05-25** — preflight discovery prod notify_db boş data state (R28 NEW + BL-028 prod data seed prereq: template + subscriber_contact + OpenFGA tuple)) |
