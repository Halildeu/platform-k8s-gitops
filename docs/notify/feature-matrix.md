...
| # | Özellik | Tier | Sub-faz | Status |
|---|---|---|---|:---:|
| O1 | Outage fallback bypass (Alertmanager direct, orchestrator bypass) | **MVP-dar** | 23.2 | 🟢 **mock-receipt mitigated, Teams-pivot reverify pending** (M3 T1.4 23.2.D — first controlled drill 2026-05-10 Mailpit SMTP-only; BL-008 mock-receipt drill 2026-05-24 Codex `019e5aaf` REVISE absorb test cluster DUAL receipt evidence (webhook-receiver POST `/slack-mock` 200 + Mailpit SMTP — same Alertmanager dispatch cycle); 10/10 mock-receipt criteria PASS; route narrowing + permanent netpol). **BL-D43-TEAMS-PIVOT 2026-05-24 (Codex `019e5ba9`)**: kullanıcı kararı sonrası Slack → Microsoft Teams Power Automate workflow webhook pivot — `slack_configs` → `webhook_configs`, `TEAMS_WEBHOOK_URL` Vault path/key rename, `perf-alerts-slack` → `perf-alerts-teams`, `direct-fallback` vendor-neutral korunur; helm-values + ESO + docs source-side closed. Test cluster Teams mock-receipt drill yeniden koşulacak (`http://webhook-receiver.platform-test:9000/teams-mock`). Real Teams Power Automate workflow setup + prod activation board issues yeniden açıldı (Slack #853/#854 kapatıldı). R-NEW R27 eklendi. |
