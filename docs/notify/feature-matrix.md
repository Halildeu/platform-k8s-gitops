...
**Status update 2026-05-24 (BL-D43-TEAMS-PIVOT, Codex `019e5ba9` REVISE/ready_for_impl absorb)**: Kullanıcı kararı "slack kullanmıyoruz teams kullanıyoruz" — D43 outage fallback Slack-varsayan helm-values + ESO config Microsoft Teams Power Automate workflow webhook'a pivot edildi (1 PR gitops/docs). §15 O1+O2 wording Slack → Teams; mock-receipt mitigation tipi-agnostik (HTTP layer eşdeğer), Teams reverify drill ayrı agent action. Board #853 + #854 kapatıldı, 2 yeni Teams issue açıldı. R-NEW R27 eklendi (Power Automate workflow lifecycle drift). A2 Slack adapter (T4.1.2 backend PR #271) **dormant kalır** — silinmedi; T4.1.2 user notification topic için ayrı yüzey ve organizasyonel kullanım yok (status 🟢 LIVE asset-preserved).
...
| A4 | SMS (JetSMS primary, NetGSM secondary) | **MVP-geniş** | 23.3 | 🟢 (**M4 prod LIVE 2026-05-20** sha-6307428 — JetSMS primary `SmsAdapter activated: primary=jetsms` + `JetSmsDlrPollingWorker scheduling=true` + DLR DELIVERED end-to-end test cluster; multipart context routing 23.3.2 LIVE; **NetGSM secondary R1 ⏳ DEFER asset-preserved** per kullanıcı kararı 2026-05-23 — sözleşme kısa vadede yok; JetSMS-only degraded mode kabul edilen kalıcı işletim durumu; NetGsmProvider + Vault/ESO dormant. Prod functional canary KC operator gate `org_id=default` ext) |
...
| B16 | Fallback channel canary test (gate) | **MVP-dar** | 23.2 | 🟢 **mock-receipt mitigated, Teams-pivot reverify pending** (M3 T1.4 23.2.D — BL-008 mock-receipt drill 2026-05-24 dual-receipt evidence (webhook-receiver POST 200 + Mailpit SMTP — same Alertmanager dispatch cycle); Codex `019e5aaf` REVISE absorb; **BL-D43-TEAMS-PIVOT 2026-05-24 (Codex `019e5ba9`)** kullanıcı Slack → Teams pivot kararı sonrası Slack #853/#854 kapatıldı, Teams Power Automate workflow setup + prod activation yeni board issues operator-external) |
...
| # | Özellik | Tier | Sub-faz | Status |
|---|---|---|---|:---:|
| O1 | Outage fallback bypass (Alertmanager direct, orchestrator bypass) | **MVP-dar** | 23.2 | 🟢 **mock-receipt mitigated, Teams-pivot reverify pending** (M3 T1.4 23.2.D — first controlled drill 2026-05-10 Mailpit SMTP-only; BL-008 mock-receipt drill 2026-05-24 Codex `019e5aaf` REVISE absorb test cluster DUAL receipt evidence (webhook-receiver POST `/slack-mock` 200 + Mailpit SMTP — same Alertmanager dispatch cycle); 10/10 mock-receipt criteria PASS; route narrowing + permanent netpol). **BL-D43-TEAMS-PIVOT 2026-05-24 (Codex `019e5ba9`)**: kullanıcı kararı sonrası Slack → Microsoft Teams Power Automate workflow webhook pivot — `slack_configs` → `webhook_configs`, `TEAMS_WEBHOOK_URL` Vault path/key rename, `perf-alerts-slack` → `perf-alerts-teams`, `direct-fallback` vendor-neutral korunur; helm-values + ESO + docs source-side closed. Test cluster Teams mock-receipt drill yeniden koşulacak (`http://webhook-receiver.platform-test:8080/teams-mock`). Real Teams Power Automate workflow setup + prod activation board issues yeniden açıldı (Slack #853/#854 kapatıldı). R-NEW R27 eklendi. |
| O2 | Notification-orchestrator down → Teams direct (was Slack) | **MVP-dar** | 23.2 | 🟢 **mock-receipt mitigated, Teams-pivot reverify pending** (T1.4 BL-008 mock-receipt drill 2026-05-24 dual-receipt evidence; **BL-D43-TEAMS-PIVOT 2026-05-24 (Codex `019e5ba9`)**: kullanıcı kararı "slack kullanmıyoruz teams kullanıyoruz" — Slack → Microsoft Teams Power Automate workflow webhook pivot; real test Teams workflow + prod activation board issues yeniden açıldı operator-external; R-NEW R27 Power Automate lifecycle drift) |
