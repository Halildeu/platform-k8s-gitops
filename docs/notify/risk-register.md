...
| R2 | KVKK erasure scope yanlış implement → audit fail / legal exposure | Medium | Critical | **High** | **Codex AI proxy review 2026-05-21 (`019e4950`)**: `PARTIAL_COMPLIANT + RISK_FLAGGED` verdict. 7 risk (3 P0 + 3 P1 + 1 P2): Madde 13 30-gün SLA + provider propagation + backup tombstone + log leakage + audit pseudonymize + tenant-scoped DPO + runbook drift. Fix PR'lar zinciri: **PR-K2+K3** (provider matrix + backup tombstone runbook) 2026-05-21 MERGED docs-only ✅; PR-K4+K7 (log redaction + runbook refresh) pending; PR-K1 (erasure ledger V18 + 30-gün SLA) backend pending; PR-K5+K6 (audit pseudonymize + tenant DPO authz) backend pending. **DPO/legal final onay**: Codex AI proxy review "audit trail accepted" ama "KVKK legal review tamamlandı" yerine geçmez; gerçek hukukçu onayı bekliyor (Office 365 Graph API admin scope, JetSMS/NetGSM DPA retention promise, webhook transitive sorumluluk, Slack/Teams workspace retention policy). Önceki: Legal review öncesi merge yasak; runbook + integration test pre-prod | legal/dev + AI proxy | 🟡 Active (impl P0 fix'ler partial closed) | 23.2.B | 2026-05-21 |
