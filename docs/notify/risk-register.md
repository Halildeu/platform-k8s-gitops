...
| R12 | Provider config rollback transaction race (atomic switch) | Low | High | **Low** | provider_config_history immutable rows + cache invalidate test; T1.3 backend Testcontainers integration test spawn_task chip 2026-05-10 06:32Z (atomic_switch + concurrent_switch_race + cache_invalidate + rollback_on_fail; user-side cross-repo platform-backend) | dev | 🟡 Active | 23.2.C | 2026-05-10 |
