...
| R24 | JetSMS VFO channel ErrorCode=04 (Biotekno OTP allowlist provisioning gap) | Medium | Medium | **Low** | PR-A3.2 cluster smoke 2026-05-20: VFO channel routing logic LIVE (`channel resolved VFO: topic_key=auth.mfa-otp` log proven) ama JetSMS provider Biotekno tarafında VFO outbound sender ID OTP allowlist provisioning gap → ErrorCode=04 reject. Workaround: tüm SMS trafik default VF channel'dan çıkar (R6 contract'a uyumlu). Mitigation: Biotekno müşteri temsilcisi ile sender ID OTP provisioning chain'i + retry; provider tarafı çözülünce VFO routing automatic devreye girer (config side hâlâ doğru). Audit: actual_channel propagation kanıtı [docs/faz-23-evidence/2026-05-20-23-3-2-jetsms-multipart-context-routing-evidence.md](../faz-23-evidence/2026-05-20-23-3-2-jetsms-multipart-context-routing-evidence.md). | ops + provider | 🟡 Active | 23.3.2 + 23.X | 2026-05-20 |
