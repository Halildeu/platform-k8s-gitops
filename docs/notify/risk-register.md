...
|---|---|:---:|:---:|:---:|---|---|:---:|---|---|
| R1 | NetGSM secondary provider sözleşme gecikmesi → SMS failover acceptance | Low | Low | **Low** | **DEFERRED — sözleşme kısa vadede yapılmayacak (kullanıcı kararı 2026-05-23)**. JetSMS-only degraded mode = **kabul edilen kalıcı işletim durumu** (JetSMS canlı sözleşme + HTTP API + DLR LIVE prod sha-6307428). NetGSM secondary failover acceptance kriteri sözleşme imzalanana kadar ertelendi — aktif risk değil, parked. **Asset-preserved (R23/ADR-0024 pattern)**: `NetGsmProvider` (REST v2) + `SmsAdapter` failover facade + Vault `kv/platform/notification-orchestrator` NetGSM keys (username/password/msgheader/dlr_token — empty fail-closed) + ESO entries dormant asset olarak **KORUNUR — kaldırılmaz / revert edilmez**. Reactivation chain (sözleşme imzalanırsa): Vault NetGSM keys seed → ConfigMap secondary enable → digest bump → failover acceptance test. | ops | ⏳ DEFER | 23.3.1 | 2026-05-23 |
...
- **2026-06-08** (23.3 SMS milestone): R3 (DKIM activation). ~~R1 (NetGSM secondary contract)~~ — R1 ⏳ DEFER 2026-05-23 (sözleşme kısa vadede yok); review gerekmiyor, sözleşme imzalanırsa reactivation.
