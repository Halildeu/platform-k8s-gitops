...
- [~] T1.4 23.2.D Outage fallback bypass D43 merged + **mock-receipt drill** LIVE — first controlled drill 2026-05-10 (Mailpit SMTP-only; Slack NXDOMAIN sentinel); **BL-008 mock-receipt drill 2026-05-24 16:14-16:26Z** (Codex `019e5aaf` REVISE absorb) test cluster DUAL receipt evidence (webhook-receiver POST `/slack-mock` 200 length=983 + Mailpit `[D43 DRILL]` 16:17:33Z — same dispatch cycle); 10/10 mock-receipt criteria PASS; route narrowing root null + regex `NotifyServiceDown|NotifyServiceAbsent`; NetworkPolicy permanent commit. R9 🟡 Partial → 🟢 **Mitigated (mock-receipt)**. Evidence: `docs/faz-23-evidence/2026-05-24-bl008-r9-d43-drill.md`. **BL-D43-TEAMS-PIVOT 2026-05-24 (Codex `019e5ba9` REVISE/ready_for_impl absorb)**: Kullanıcı kararı "slack kullanmıyoruz teams kullanıyoruz" — Slack-varsayan helm-values + ESO config Microsoft Teams Power Automate workflow webhook'a pivot (1 PR gitops/docs source-side closed: `slack_configs` → `webhook_configs`, `SLACK_WEBHOOK_URL` → `TEAMS_WEBHOOK_URL`, `perf-alerts-slack` → `perf-alerts-teams`, `direct-fallback` vendor-neutral korunur). Slack adapter (T4.1.2 backend PR #271) dormant kalır (asset-preserved). R-NEW R27 eklendi (Power Automate workflow lifecycle drift). **Teams reverify pending**: test cluster Teams mock-receipt drill (`webhook-receiver/teams-mock` endpoint) ayrı; **Real Teams Power Automate workflow setup + prod activation** yeni board issues operator-external (Slack #853/#854 kapatıldı 2026-05-24).
...
- [x] Risk register: R2 🟢 Mitigated (Codex `019e5189` legal verdict 2026-05-23), R9 🟢 **Mitigated (mock-receipt, Teams-pivot reverify pending)** (BL-008 2026-05-24 Codex `019e5aaf` mock dual-receipt drill + BL-D43-TEAMS-PIVOT 2026-05-24 Codex `019e5ba9` Slack → Teams pivot; real Teams Power Automate workflow setup + prod activation new board issues ayrı operator-external — Slack #853/#854 kapatıldı), R12 🟢 mitigated, R13 🟢 mitigated, R19 🟢 mitigated, **R23 🟡 active monitored** (Graph mail adapter deferred; SMTP canonical; Entra asset preserved; reactivation chain documented in ADR-0024 + RB-graph-mail-adapter-activation.md + #892), **R27 🟡 Active (mitigation defined)** Power Automate workflow lifecycle/owner/policy drift breaks D43 outage Teams channel (BL-D43-TEAMS-PIVOT 2026-05-24 Codex `019e5ba9` — service-account flow + exported package backup + monthly synthetic Teams smoke + defense-in-depth Teams+SMTP+GitHub Issue)
