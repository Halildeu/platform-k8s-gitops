...
| P3 | endpoint-admin MANAGER token (revoke/dispatch için) | `GET /api/v1/admin/endpoint-devices` 200 | operatör (kullanıcı yetkili token) |
| P4 | GPO link mevcut (pilot OU) — GPO rollback adımı için | `gpresult /r` cihazda policy görünür | operatör (AD) |
| P5 | `wave-preflight.ps1` cihaza kopyalandı | dosya mevcut, PSScriptAnalyzer temiz | agent |
| P6 | Backup/recovery: cihazın system restore point veya VM snapshot | snapshot alındı | operatör |

> **No Fake Work**: P1/P5 agent-doable + LIVE; P2/P3/P4/P6 operatör-gated. Bu runbook drill'i sıralar; canlı koşum P2-P6 hazır olunca yapılır.
...
msiexec /i "EndpointAgent-<ver>-signed.msi" /qn /norestart `
  ENROLL_RESPONSE_FILE="C:\ProgramData\EndpointAgent\enroll-response.json" `
...
2. `endpoint-agent.exe service uninstall --name EndpointAgent` (maintenance token verildiyse iletilir).
3. Fallback: `Stop-Service -Force` + `sc.exe delete EndpointAgent`.
...
5. Config store (`config\hmac-credential.dpapi`) **korunur** (`-RemoveConfig`/`PURGE_CONFIG=1` verilmedikçe).
...
**Komut** (`wave-preflight.ps1` ile, veya manuel):
```powershell
# wave-preflight.ps1 rollback-verify modu (uninstall sonrası temiz state)
powershell -NoProfile -ExecutionPolicy Bypass -File .\wave-preflight.ps1 -Mode rollback-clean -Json
...
**Beklenen** (uninstall sonrası temiz state): service yok, scheduled task yok, HKLM `Services\EndpointAgent` yok, exe yok, **log dizini korunur** (evidence retention), config store `-RemoveConfig` verilmediyse korunur.

**Fail-sinyali**: orphan service (`sc query EndpointAgent` STATE!=1060), orphan scheduled task, stale HKLM `Environment` regkey kalıntısı, exe locked (`Wait-AgentProcessExit` timeout → manuel `taskkill /F /IM endpoint-agent.exe`).
...
**Mekanizma** (backend doğrulandı — `EndpointDeviceLifecycleService`, Codex thread `019ea789`): enrollment revoke = **device decommission** (KVKK: deactivate-not-delete, reversible). Decommission cascade'i **pending agent work'ü iptal eder** (commands / secrets / maintenance tokens / open uninstall requests) ve "decommissioned device cannot act OR revive itself" invariant'ı agent/auth/enrollment guard'larında enforce edilir.
...
# 3b. "Komut alamaz" verify — revoked cihaza yeni install command POST et
curl -sS -X POST "$BASE/api/v1/admin/endpoint-devices/$DEVICE_ID/commands" \
  -H "Authorization: Bearer $MGR_TOKEN" -H "Content-Type: application/json" \
  -d '{"commandType":"COLLECT_INVENTORY"}' -o /dev/null -w "%{http_code}\n"
# beklenen: 4xx (decommissioned device dispatch reddedilir) — RAW komut çıktısı loglanmaz
...
**Beklenen**: decommission → `DECOMMISSIONED`; revoked cihaza komut dispatch 4xx; reactivate → `OFFLINE`/`PENDING_ENROLLMENT`; reactivate sonrası gerçek heartbeat → `ONLINE`.

**Fail-sinyali**: decommission sonrası cihaz hâlâ komut alıyor (cascade/guard bug → backend triage, M7 stop); reactivate 409 (lifecycle state mismatch).

**Devam eşiği**: revoke → komut-alamaz → reactivate zinciri yeşil → D4.

> **Audit kanıtı**: decommission/reactivate her ikisi de `endpoint_device_lifecycle_audit` (who/when/why + cascade counts) + hash-chained `endpoint_audit_events` (`ENDPOINT_DEVICE_DECOMMISSIONED`/`ENDPOINT_DEVICE_REACTIVATED`) yazar. Drill evidence'ında bu row'lar (PII-redacted) referanslanır.
...
# 4c. Assigned-MSI ise: unlink sonrası bir sonraki gpupdate'te MSI otomatik uninstall
#     (per-machine assigned package "remove when falls out of scope" davranışı)
gpresult /r                # GPO artık uygulanmıyor doğrula
```

**Beklenen**: unlink sonrası GPO cihazda görünmez (`gpresult /r`); assigned-MSI senaryosunda paket scope-dışı kalınca uninstall; propagation süresi kaydedilir.

**Fail-sinyali**: unlink sonrası MSI hâlâ reinstall ediliyor (loop guard — accidental ikinci link?); propagation > pencere.
...
**Mekanizma doğrulaması**: endpoint-admin'de **global tek-tık dispatch kill-switch endpoint'i YOK** (kaynak doğrulandı). Backend pause bu nedenle **operasyonel + kademeli**:

| Pause katmanı | Mekanizma | Komut |
|---|---|---|
| **1. Yeni komut üretimini durdur** | Admin-initiated dispatch (`POST /endpoint-commands`) durdurulur; agent yalnız `GET /api/v1/agent/commands/next` ile poll eder — kuyruğa yeni komut girmezse hiçbir cihaz yeni iş almaz | (operasyonel — yeni POST atma) |
| **2. Rollout-ring de-assign** | Dalga cihazlarını aktif ring dışına al → ring-filtreli komutlar onlara gitmez | `PATCH /api/v1/admin/endpoint-devices/{id}/rollout` body `{"deploymentRing":"PILOT"}` (dar ring'e geri çek) |
| **3. Etkilenen subset'i decommission** | Belirli cihazları hızla devre dışı (D3 mekanizması) — pending komutları cascade-cancel | `POST .../{id}/decommission` |

```bash
# Katman 2 örneği — dalga subset'ini PILOT ring'e geri çek (genişlemeyi durdur)
curl -sS -X PATCH "$BASE/api/v1/admin/endpoint-devices/$DEVICE_ID/rollout" \
  -H "Authorization: Bearer $MGR_TOKEN" -H "Content-Type: application/json" \
  -d '{"deploymentRing":"PILOT","deviceTags":["m7-rollback-paused"]}' | jq '.deploymentRing'
```

**Beklenen**: pause sonrası yeni install command üretilmiyor; etkilenen cihazlar bir sonraki poll'da yeni iş almıyor.

**Fail-sinyali**: pause sonrası cihazlar hâlâ yeni komut çekiyor (ring filtre / cascade bug).

**Devam eşiği**: pause etkisi gözlendi → D6.

> **Gelecek hardening (board backlog)**: explicit `POST /api/v1/admin/rollout/pause` global kill-switch endpoint'i M6/M7 öncesi eklenebilir (şu an operasyonel pause yeterli ama tek-tık atomic değil). Bu runbook mevcut gerçek yüzeyi belgeler; uydurma endpoint YAZMAZ.
...
|---|---|
| Runbook + komut + beklenen + fail-sinyal + evidence template | 1 pilot cihaz + VM snapshot (P2/P6) |
| `wave-preflight.ps1 -Mode rollback-clean` | endpoint-admin MANAGER token (P3) |
| Signed MSI (AG-018 LIVE) | GPO link + AD action (D4, P4) |
| Backend revoke/pause mekanizma doğrulaması (D3/D5 endpoint'ler gerçek) | Canlı drill koşumu + evidence doldurma |
