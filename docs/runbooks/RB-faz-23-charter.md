...
| 23.2 | Production MVP dar | **🟢 source-ready + acceptance candidate** Session 44 2026-05-10 (FULL ACCEPTANCE state achieved post-merge of PR #149 + #503) — 5-state matrix at PR-time: Source 12/12 + Live 12/12 + Evidence 12/12 + Acceptance 11/12 + Blocked 0/12 → **post-merge 12/12 + 6/6 sub-faz fully 🟢** (23.2.A T1.1 trilogy MERGED 3/3 + 23.2.B subscriber self-service + 23.2.C provider config rollback R12 MITIGATED T1.3 + 23.2.D outage fallback **R9 MITIGATED first drill** + 23.2.E **data classification 9-test acceptance candidate (PR #149)** + 23.2.F abuse guards FULL acceptance); R2 KVKK legal review CLOSED 2026-05-23 (Codex `019e5189` final legal verdict AGREE) — 23.2 full 🟢 done; K6 P1 non-blocking 23.2.B follow-up |
| 23.3 | Production MVP geniş | **🟢 source-ready + acceptance candidate** Session 47 2026-05-20 — prod cutover LIVE (PR-B1+B2+B3+B4 zincir MERGED + pod LIVE 1/1 sha-6307428 + ProductionConfigValidator all PASSED); residual ext: R24 Biotekno OTP allowlist + DKIM tenant enable + DNS CNAME + prod canary smoke + 72h observation. **R1 NetGSM secondary ⏳ DEFER** — sözleşme kısa vadede yapılmayacak (kullanıcı kararı 2026-05-23); JetSMS-only degraded mode kabul edilen işletim durumu; NetGsmProvider + Vault/ESO asset-preserved |
...
| 23.7 | v1 — push (FCM/APNS) | code | 2 hafta | 23.4 + Faz 22.2 | 🟡 partial (WebPush browser-only **LIVE end-to-end** 2026-05-23 — PR-W1..W7 + #648/#649 frontend + #652 RTK fetchfn fix + #986/#987 deploy + #990 OpenFGA model extension safe-phase + #995 model_id cutover + #996 internal-API-key ESO re-align; subscribe akışı browser-kanıtlı **RB-webpush §3.10 ✅**; SUCCESS push delivery proven (intent COMPLETED + `notify_dispatch_outcome_total{channel="push",status="DELIVERED"} 1.0` + FCM 201 msg_id) **RB-webpush §3.11 ✅**; mobil FCM/APNS Faz 22.2 dep DIŞI — partial kalıyor çünkü orijinal kapsam mobile push, browser-only delivered subset) |
...
- **Out of M6 scope**: NetGSM secondary R1 ⏳ DEFER asset-preserved (kullanıcı kararı 2026-05-23); prod SMS functional canary / DLR terminal status evidence ext-gated (KC operator gate `org_id=default` claim setup, RB-prod-canary-kc-claim-setup.md).
