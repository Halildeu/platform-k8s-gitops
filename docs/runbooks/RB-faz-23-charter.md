...
| 23.3 | Production MVP geniş | **🟢 infrastructure LIVE; 🟡 functional data seed pending** (2026-05-25 marker daraltma — Codex `019e5e76` iter-2/iter-3/iter-4 absorb) — prod cutover infrastructure LIVE (PR-B1+B2+B3+B4 zincir MERGED + pod LIVE 1/1 sha-6307428 + ProductionConfigValidator all PASSED); **BL-010 prod KC `serban` realm ✅ COMPLETED 2026-05-25** (PR #1062 — notify-canary scope + org_id mapper + persona LIVE + JWT 3-way claim verified); **functional canary BLOCKED by prod notify_db data seed** (R28 NEW Pending — `notification_template active=true` 0 rows + `subscriber_contact` 0 rows; BL-028 mitigation: template + subscriber_contact + OpenFGA tuple seed). BL-011 SMS canary DEFER until BL-028 complete. External residual: R24 Biotekno OTP allowlist + DKIM tenant enable + DNS CNAME (BL-009 trigger-based DEFER 2026-05-25 PR #1061) + 72h observation. **R1 NetGSM secondary ⏳ DEFER** — sözleşme kısa vadede yapılmayacak (kullanıcı kararı 2026-05-23); JetSMS-only degraded mode kabul edilen işletim durumu; NetGsmProvider + Vault/ESO asset-preserved |
