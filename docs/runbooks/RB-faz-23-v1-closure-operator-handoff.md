...
- [ ] **BL-011** Prod SMS functional canary smoke — canonical canary example: `RB-prod-canary-kc-claim-setup.md` (canonical payload contract: `topicKey` + `recipients` + `template` + `channels` + `orgId`) — **DEFER 2026-05-25 (preflight discovery + Codex `019e5e76` iter-2 AGREE)**: prod `notify_db` data seed eksik (BL-028 / R28 prereq). 5 trigger condition: (a) BL-028 prod data seed COMPLETED (template + subscriber_contact + OpenFGA tuple), (b) backend functional canary smoke 202 + audit row (no real SMS), (c) operator window scheduled, (d) recipient `+905551815564` re-confirm, (e) cost cap ≤3 SMS confirm.
