...
3. **F2-B 2-fazlı certreq flow (iter-2 absorb)**:
   ...
   - **Faz 2 (pending retrieve)** — eğer JSON state'de pending entry varsa (duplicate guard: yeni submit YASAK):
     ...
     - `certreq -retrieve -config $CAConfig $RequestId $cer` — CA'dan cert hazır mı çek
       ...
       - Denied (API `CR_DISP_DENIED`=2 / CA DB `Disposition` column=8; certreq -retrieve exit non-zero + output contains "denied" text — F1-A absorb iter-5/6: HRESULT 0x80094004 != denied semantik; denied state iki ayrı disposition katmanında farklı code: API CR_DISP_DENIED=2, CA DB Disposition column=8 — certutil -view -restrict 'Disposition=8' kullanılır) → error log + pending state remove (next run yeni submit eder)
...
| `Denied state, "denied" text in certreq output` | — | Operator CA Manager queue'da request'i deny etti (manuel inspection sonrası — örn. requested adcomputer GUID match check fail). **Disposition iki katmanda (F1-A iter-6)**: API `CR_DISP_DENIED` = `2`; CA DB `Disposition` column = `8` (certutil -view -restrict 'Disposition=8'). | F2-B: pending state cleanup yapar (script log'da "F2-B DENIED" görünür) → sonraki run yeni submit eder. Eğer aynı PC sürekli deny ediliyorsa root cause analysis gerek (PC OU dışında mı, machine objectGUID rotated mı, custom policy module reject mi). |
