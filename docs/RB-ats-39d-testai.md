...
- **Faz 25 Full ATS üç-artifact compensator:** #2617 merge SHA'sında çalışan `faz25-fullats-live-browser-acceptance.yml`, exact runtime veya browser kabulü düşerse GitHub App kimliğiyle merged #2617 değişikliğinin deterministik `git revert` PR'ını açar ve normal zorunlu kontrollerden sonra auto-merge ister. Compensator yalnız workflow SHA'sı #2617 `merge_commit_sha` ile ve `origin/main` head'iyle birebir aynıysa çalışır; main ilerlediyse veya revert conflict ürettiyse sessiz/parsiyel rollback yapmaz, yeni incelenmiş scope ister. Revert tek changeset içinde ATS `sha256:dce33483…72515`, permission-service `sha256:3a202b36…a5256` ve frontend `sha-653752b@sha256:28da39d9…6b2d` önceki setini birlikte restore eder; `kustomize build` bu üçlüyle geçmeden PR açılmaz. Doğrudan cluster patch'i yoktur. Bu mekanizma test-only'dir; production rollback otoritesi değildir.
