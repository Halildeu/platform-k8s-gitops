...
### Faz 25 test model-governance ilk geçişi (#2526)

- `live-stt` boot gate üç exact desired-state bağı ister: `ATS_AI_ENDPOINT_REF=faz24-stt-prod`, `ATS_AI_APPROVAL_TRANSCRIBE_REF=mapr_549a8e…a732d` ve aynı katalog kaydını taşıyan immutable ATS image digest. Activation ConfigMap, canlı Deployment image ve operator image birebir eşleşmeden script çalışmaz.
- Flyway→boot-gate sırası ATS source `6af613a` içinde explicit bean bağıdır: `WiringConfig.flyway(DataSource)` önce `flyway.migrate()` çağırır; `modelGovernanceLedgerReader(DataSource, Flyway)` bu bean'e, registry reader'a ve `authorizedModelBindings` registry'ye bağımlıdır. Bu nedenle V6 ilk fail-closed boot'ta yazılabilir; boş ledger gate'i bundan sonra composition/Ready'yi engeller. Workflow önce V6'yı canlı DB'de kanıtlar, sonra append yapar, en son Ready bekler.
...
- Dış check confirmation: `CHECK_FAZ25_TEST_MODEL_GOVERNANCE`; append confirmation: `APPEND_FAZ25_TEST_MODEL_GOVERNANCE`. Append sabit `mgt_25260000-0000-4000-8000-000000000001` transition kimliği + opak `cross-ai/faz25/2526` actor ref kullanır. Tekrar koşum yeni satır üretmez; canonical CLI idempotent replay döndürür.
...
- Append sonrası kabul: CLI projeksiyonu `APPROVED idempotent=true`; DB'de yalnız sequence `0`, `UNINITIALIZED→APPROVED`, exact actor/reason, genesis previous-hash ve CLI çıktısıyla aynı 64-hex entry-hash. Test-PG append-only mekanizma kanıtıdır; mevzuatsal/harici WORM depolama iddiası değildir.
