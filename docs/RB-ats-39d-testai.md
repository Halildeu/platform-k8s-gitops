...
- Ürün kabulü `.github/workflows/faz25-fullats-live-browser-acceptance.yml` ile exact ATS + permission + frontend digest'lerine ve exact frontend source SHA'ya bağlanır. Browser zinciri İK login → kalıcı taslak/düzenleme/önizleme/yayın → dinamik public ilan → adayın versioned CV aydınlatmasını açıkça onaylaması → gerçek sentetik PDF'yi candidate-only bounded ATS parser'a yüklemesi → provenance gösterilen alanlardan birini düzenlemesi, birini reddetmesi, kalan güvenli önerileri kabul etmesi → yalnız seçilen alanları atomik taslağa aktarması ve reddedilen zorunlu alanı elle tamamlaması → form önizlemesi/ayrı açık onay/kalıcı makbuz → İK inbox → yapılandırılmış insan değerlendirmesi → insan kontrollü mülakat aşaması → kalıcı görüşme planı ve adaya güvenli takvim → atanmış insan scorecard'ı ve görüşme tamamlama → kalıcı teklif taslağı ve insan onaylı iletim → adayın teklif kabulü → İK'nın insan kontrollü işe alım sonucu → adayın `İşe alındı` sonucunu görmesi → ilanı duraklat/yeniden yayınla/kapat → duraklatılmış/kapalı ilanda yeni POST 404 + `NOT_FOUND` → mevcut aday makbuzunun yaşamaya devam etmesidir. Network evidence header/body/raw PDF tutmaz; sentetik PDF yalnız parser upload isteğinde geçici taşınır ve final application/evidence payload'ına giremez. API smoke, yalnız update yanıtı veya local UI testi bu browser kanıtı yerine geçmez.
...
- **Faz 25 Full ATS ATS + frontend runtime compensator:** #2685'in exact merge SHA'sında çalışan `faz25-fullats-live-browser-acceptance.yml`, runtime veya gerçek browser kabulü düşerse GitHub App kimliğiyle ATS pinini reviewed base'deki `sha256:8812ab4e…66a11`, frontend pinini `sha-eee1310@sha256:46a55e16…6402` artifactine döndüren ve `fullats-promotion-state.txt=ROLLED_BACK` yapan üç dosyalık PR açar. Permission-service `sha256:55f2f2f2d1edb3aa67c663c1411b0cc21ab1818d10b4d8d70a5beeeb32ade13d` korunur. Compensator ancak failed workflow SHA, current main ve #2685 merge SHA birebir aynıysa; #2685 tek-parent squash commit'i doğrudan reviewed `62f763ba1e4334588fbc9b250a665bacf369a263` base'in çocuğu, squash tree'si exact PR head tree'si ve PR gövdesindeki yalnız direct Codex `single` AGREE receipt'i aynı head/scope ile bağlıysa çalışır. Claude/MiniMax, legacy risk-trigger veya eski-head receipt kabul edilmez. Trusted-base verifier exact üç-path diff'i, restored ATS + frontend bloblarını ve marker'ı doğrulayıp base/head'e bağlı attestation üretmeden automation istisnası geçmez. Bot PR required check'leri bekler, head'i yeniden doğrular, `--match-head-commit` ile normal korumalı squash merge yapar; `--admin` yoktur. Ardından exact rollback merge revision'ı `Synced/Healthy`, üç Deployment/Ready-pod digest'i, public `build-info.json` eski frontend SHA'sı ve eski ATS digest'iyle tam D29 matrisi yeniden kanıtlanır. Merge veya post-rollback kanıtı düşerse workflow kırmızı kalır. Doğrudan cluster patch'i ve production rollback yetkisi yoktur.
