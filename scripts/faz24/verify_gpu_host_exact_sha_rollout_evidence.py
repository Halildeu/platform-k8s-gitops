...
def verify(data: dict[str, Any], expected_commit: str) -> None:
    ...
    require(live_health.get("status") in ("ok", "loading"), "live STT health is degraded")
    readiness = object_field(object_field(data, "readiness"), "liveStt")
    require(readiness.get("reachable") is True, "streaming readiness is unreachable")
    require(readiness.get("httpStatus") == 200, "streaming readiness HTTP status is not 200")
    require(readiness.get("status") == "ready", "streaming models are not ready")
    require(readiness.get("runtimeCommit") == expected_commit, "streaming runtime commit mismatch")
    require(readiness.get("preloadEnabled") is True, "streaming preload is not enabled")
    require(readiness.get("workersHealthy") is True, "streaming workers are unhealthy")
    roles = object_field(readiness, "roles")
    require(set(roles) == {"live", "final"}, "streaming roles mismatch")
    for role in ("live", "final"):
        require(roles.get(role) == "ready", f"streaming {role} role is not ready")
    runtime = object_field(readiness, "runtime")
    for role, device, compute_type in (
        ("legacy", "cpu", "int8"), ("live", "cuda", "int8"), ("final", "cuda", "float16")
    ):
        profile = object_field(runtime, role)
