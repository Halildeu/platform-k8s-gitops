...
class DeploymentEvaluator:
    def __init__(
        ...
        *,
        ...
        if mode not in {"observe", "enforce"}:
            reject("EVALUATOR_MODE_INVALID", "evaluator mode must be observe or enforce")
    ...
    def _policy_binding(self, request: DeploymentProtectionRequest) -> None:
        if request.repository_id != self.policy.repository_id or request.repository != self.policy.repository:
            reject("POLICY_REPOSITORY_MISMATCH", "webhook repository is not allowlisted")
        if request.environment != self.policy.environment:
            reject("POLICY_ENVIRONMENT_MISMATCH", "webhook Environment is not allowlisted")
        if request.installation_id not in self.policy.allowed_installation_ids:
            reject("POLICY_INSTALLATION_MISMATCH", "GitHub App installation is not allowlisted")
    ...
    def _environment_binding(self, value: dict[str, Any]) -> int:
        ...
        if not isinstance(rules, list):
            reject("ENVIRONMENT_SNAPSHOT_INVALID", "Environment protection rules are missing")
        ...
        for rule_value in rules:
            ...
            if rule_type == "required_reviewers":
                ...
                app = self._object(rule.get("app"), "environment.protection_rules[].app")
        ...
        if len(custom_apps) != 1:
            reject("ENVIRONMENT_CUSTOM_RULE_DRIFT", "v1 requires exactly one custom rule App")
        ...
        if self.policy.phase == "dual-gate" and not required_reviewer:
            reject("ENVIRONMENT_REVIEWER_DRIFT", "dual-gate phase requires a human reviewer")
        ...
        branch = self._object(value.get("deployment_branch_policy"), "deployment_branch_policy")
        if branch.get("protected_branches") is not False or branch.get("custom_branch_policies") is not True:
            reject(
    ...
    def _run_binding(
        ...
        *,
        ...
        if run.get("head_branch") != expected_branch:
            reject("INTENT_REF_MISMATCH", "workflow run did not use the immutable intent ref")
        repository = self._object(run.get("repository"), "run.repository")
        head_repository = self._object(run.get("head_repository"), "run.head_repository")
        if (
            ...
            reject("RUN_REPOSITORY_MISMATCH", "run repository or head repository differs")
        ...
        if actor_id not in self.policy.allowed_dispatcher_actor_ids:
            reject("RUN_ACTOR_MISMATCH", "triggering actor is not the trusted dispatcher")
        ...
        if status not in {"queued", "waiting", "pending", "in_progress"}:
            reject("RUN_STATE_INVALID", "workflow run is not awaiting or entering execution")
        ...
        if created_at < finalized_at or created_at > finalized_at + MAX_DISPATCH_DELAY:
            reject("RUN_DISPATCH_WINDOW_INVALID", "workflow run is outside dispatch window")
        if created_at > self._now() + timedelta(seconds=60):
            reject("RUN_DISPATCH_WINDOW_INVALID", "workflow run creation time is in the future")
    ...
    def evaluate(self, request: DeploymentProtectionRequest) -> EvaluationResult:
        ...
        if (
            ...
            reject("INTENT_WEBHOOK_BINDING_MISMATCH", "registered intent differs from webhook")
        ...
        if subject["deploymentClass"] not in self.policy.allowed_deployment_classes:
            reject("HUMAN_REQUIRED_CLASS", "deployment class is outside reversible non-prod")
        ...
        if grant_end - grant_start > timedelta(minutes=self.policy.max_grant_ttl_minutes):
        ...
        if repository.get("id") != self.policy.repository_id or repository.get("full_name") != self.policy.repository:
        ...
        if live_ref.head_sha != request.head_sha or live_ref.ref_object_id != record.ref_object_id:
            reject("INTENT_REF_MOVED", "live intent ref no longer resolves to reviewed head")

        matching_stages = [
            stage for stage in verified.payload["workflowStages"]
        ...
        if tuple(signed_stage["runsOnLabels"]) != stage_policy.required_runs_on_labels:
            reject("RUNNER_POLICY_OR_INPUT_AUTHORITY_MISMATCH", "signed runner labels differ")
        if stage_policy.require_runner_group != ("runnerGroupId" in signed_stage):
            reject("RUNNER_POLICY_OR_INPUT_AUTHORITY_MISMATCH", "signed runner group differs")
        ...
        if grant["triggeringActorId"] != self._object(run["triggering_actor"], "actor")["id"]:
