...
for repo_dir in "$LEDGER_DIR"/*/; do
  ...
  for ledger_file in "$repo_dir"/*.json; do
    ...
    # Branch identity carries the service: one commit can now hold several
    # per-service entries (gitops#3677) and each must reach its own draft PR.
    branch="auto-promotion/prod-${repo}-${service}-${short_sha}"
