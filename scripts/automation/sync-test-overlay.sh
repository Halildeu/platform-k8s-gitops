...
# ------------------------------------------------------------
# Diff guard — the PR may change ONLY this file, ONLY digest: lines, ONLY <= 13
# ------------------------------------------------------------
...
if [[ "$added" -lt 1 || "$added" -gt 13 ]]; then
  echo "::error::[sync-test-overlay] diff-guard: ${added} digest line(s) changed (expected 1..13 backend services)"
  exit 1
fi
