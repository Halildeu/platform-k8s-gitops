...
# Board #2500 — resolve the TARGET branch's remote tip explicitly.
#
# The push below is a force-push to a rolling automation branch. Bare
# `--force-with-lease` derives its expected value from the remote-tracking ref
# `refs/remotes/origin/$BRANCH`, which this script never fetches — only
# `origin main` is fetched above. That made the safety of the lease depend on
# the CALLER's checkout:
#   * `deploy-backend-testai.yml` uses the default (shallow, single-branch)
#     checkout, so `refs/remotes/origin/$BRANCH` is typically ABSENT — the lease
#     has no baseline to compare against.
#   * A caller that happens to have a stale copy of the ref gets a lease that
#     compares against a value someone else already superseded.
# Either way the guard the flag advertises is not the guard actually applied.
#
# `git ls-remote` asks the remote for the ref's CURRENT value at push time, and
# the explicit `--force-with-lease=<ref>:<sha>` form pins the comparison to that
# value. The lease is then a real compare-and-swap regardless of local refs.
# Same pattern as scripts/promotion/scan-promotion-candidates.sh.
#
# Fail-closed: if the remote cannot be queried we do NOT fall back to an
# unguarded push — an unreachable remote is exactly when a blind overwrite is
# most likely to clobber concurrent work.
TARGET_REF="refs/heads/${BRANCH}"
if ! REMOTE_TIP=$(git ls-remote --refs origin "$TARGET_REF" 2>/dev/null | awk 'NR == 1 { print $1 }'); then
  echo "::error::[sync-test-overlay] cannot read remote state for $TARGET_REF — refusing to push without a lease baseline"
  exit 1
fi
# Empty REMOTE_TIP = branch does not exist yet; `<ref>:` is the explicit
# "must not exist" lease, so a racing creator is still detected.
LEASE="${TARGET_REF}:${REMOTE_TIP}"
...
# idempotent-update mechanism. NOT main/master — HARD RULE respected.
#
# #2500: the lease is the EXPLICIT `<ref>:<sha>` form resolved from `git
# ls-remote` above, not the bare flag. The bare form reads
# `refs/remotes/origin/$BRANCH`, which this script never fetches, so under the
# deploy workflow's shallow checkout it had no baseline at all — the flag
# claimed a compare-and-swap it was not performing. With the explicit lease a
# concurrent writer is genuinely detected, and the push is rejected rather than
# silently clobbering their commit.
if ! git push --force-with-lease="$LEASE" origin "HEAD:${TARGET_REF}" --quiet; then
  echo "::error::[sync-test-overlay] lease rejected for $TARGET_REF — the branch moved after ls-remote (concurrent writer). Nothing was overwritten; re-run to rebuild on the new tip."
  exit 1
fi
