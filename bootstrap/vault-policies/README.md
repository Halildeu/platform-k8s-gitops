...
| `meeting-analysis-capability-writer-test` | `test/` | Faz 24 shared analysis capability trust-root first activation (`create/read`, no `update/delete`); exact-property restriction is an audited wrapper guard, not a Vault ACL boundary | SADECE test Vault |
