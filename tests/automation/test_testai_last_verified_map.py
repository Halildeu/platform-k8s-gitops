...
class LatestVerifiedMapTests(unittest.TestCase):
    ...
    def test_newest_pass_diagnostic_artifact_wins(self):
        ...
        fj, fb = self._fetchers([
            (30, [("testai-backend-acceptance-abc-30-1", False, zipped(mod.RUNTIME_REPORT, report("PASS", newer)))]),
            (20, [("testai-backend-acceptance-abc-20-1", False, zipped(mod.RUNTIME_REPORT, report("PASS", older)))]),
        ])
    ...
    def test_superseded_expired_and_broken_artifacts_are_skipped(self):
        ...
        fj, fb = self._fetchers([
            ...
            (37, [("testai-backend-acceptance-x-37-1", False, zipped("other.json", report("PASS", good)))]),
            (36, [("testai-backend-acceptance-x-36-1", False, zipped(mod.RUNTIME_REPORT, report("PASS", good)))]),
        ])
    ...
    def test_prefix_matches_the_workflows_successful_artifact_name(self):
        ...
        good = {"auth-service": "sha256:" + "1" * 64}
        fj, fb = self._fetchers([(2, [("testai-backend-diagnostic-x-2-1", False, zipped(mod.RUNTIME_REPORT, report("PASS", good)))])])
        self.assertIsNone(mod.latest_verified_map(fj, fb, "o/r", "wf.yml"))

    def test_no_usable_evidence_is_none(self):
        fj, fb = self._fetchers([(1, [("testai-backend-acceptance-x-1-1", False, zipped(mod.RUNTIME_REPORT, report("FAIL", D)))])])
        self.assertIsNone(mod.latest_verified_map(fj, fb, "o/r", "wf.yml"))
        fj, fb = self._fetchers([])
        self.assertIsNone(mod.latest_verified_map(fj, fb, "o/r", "wf.yml"))
