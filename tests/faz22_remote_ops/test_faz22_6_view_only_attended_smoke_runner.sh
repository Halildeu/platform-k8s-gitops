...
grep -q "DENETIM_SSH_TARGET=\"\${DENETIM_SSH_TARGET:-denetimpc@10.99.0.2}\"" "$SCRIPT"
grep -q "DENETIM_SSH_OPTS=\"\${DENETIM_SSH_OPTS:--i /home/halil/.ssh/id_denetim -o IdentitiesOnly=yes}\"" "$SCRIPT"
grep -q 'denetim-ssh-key-not-readable' "$SCRIPT"
grep -q "ssh -G \"\$DENETIM_SSH_TARGET\"" "$SCRIPT"
