...
"""
    ...
    def test_questions_and_cancellation_have_persistent_readback_boundaries(self):
        for marker in (
            ...
            # #240 B: cevap yolculuğu makine-zorlamalı — sorular başvuru anında yaşar,
            # zorunlu soru önizlemeyi bloklar, cevaplar kimliğe (etikete değil) bağlanır.
            "question deletion changed surviving ids",
            "question required flag persistence failed",
            "Yanıtlanması zorunlu",
            "candidate questions are not rendered in recruiter order",
            "preview opened with a required question unanswered",
            "candidate-question-${singleChoiceQuestionId}-${remoteOptionId}",
            "'answers',",
            ...
            "recruiter candidate answers section",
            "recruiter candidate answers row count mismatch",
            "recruiter-application-answer-${singleChoiceQuestionId}",
            "recruiter-application-answer-${textQuestionId}",
            "recruiter answer carries the question text",
            "recruiter answer option label",
            "recruiter answer kind and required marker",
            "Tek seçim · zorunlu",
            "recruiter second answer carries its question text",
            "recruiter second answer kind marker",
            "recruiter answer text",
            "recruiter candidate answers not in recruiter order",
            "leakableIds.length < 4 || leakableIds.some((id) => answersText.includes(id))",
            "recruiter candidate answers leak ids instead of labels",
            "recruiter candidate answers show an unanswered row for answered questions",
    ...
    def test_fullats_live_browser_is_bound_to_three_exact_runtime_artifacts(self):
        expected = {
            "ats": "sha256:c2baf0e6cedeada526f4f9814727bbd002ff4eb6b7f6d7dd9138c2977b7f5cec",
    ...
    def test_fullats_promotion_or_rollback_state_binds_exact_frontend_and_current_backends(self):
        ...
        current_ats = "sha256:c2baf0e6cedeada526f4f9814727bbd002ff4eb6b7f6d7dd9138c2977b7f5cec"
