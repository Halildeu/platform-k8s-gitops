...
| D40 | TR SMS provider native Java adapter (tier v1) | NetGSM / İletimerkezi / Mutlucell adapter'ları Spring Boot içinde Java client (TS plugin değil). `SmsProvider` interface (`send`, `queryDelivery`, `normalizeError`, `supportsUnicode`, `senderId`). **Primary**: NetGSM, **secondary**: İletimerkezi (failover: pre-accept fail otomatik secondary, kabul belirsiz manuel). DLR (Delivery Receipt) callback endpoint. GSM-7/UCS-2 segment + Türkçe karakter + sender ID. **Tier v1** (Codex `019df86f` Q2 REVISE — MVP'den çıkarıldı, MVP-geniş 23.3'e taşındı). **D40-IYS sub-faz drift**: IYS (İleti Yönetim Sistemi) lookup, ticari mesajda zorunlu, OTP/transactional muaf. |
