...
| **22.6** Remote Access Bridge | Agent-initiated outbound remote access / reverse tunnel / session broker hattı. Faz 22.5 command polling yerine geçmez; interaktif ve yüksek yetkili destek oturumları için ayrı güvenlik modeli üretir. | Evidence-driven | Sensitive Endpoint Ops Governance Gate (#1388); broker ADR + state machine (#524); agent outbound tunnel spike (#116); remote-access umbrella (#510) | Canonical plan: [`docs/faz-22-remote-access-bridge-plan.md`](./docs/faz-22-remote-access-bridge-plan.md). Inbound port açılmaz; session token TTL, dual-control, RBAC, audit/recording ve retention olmadan runtime yok. | 22.5 ile paralel planlanabilir; runtime #1388 gate'e bağlı | 🔴 PLANNING / BLOCKED by #1388 |
