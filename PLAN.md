...
| **24** Meeting Intelligence / STT Platform | Workcube ERP'ye entegre toplantı zekâsı — telefon/masaüstü/Teams ses kaynaklarından canlı transkript + konuşmacı ayrımı + özet/karar/aksiyon çıkarımı. STT compute worker (`platform-ai` Python servisleri) + Spring Boot orchestration (`audio-gateway-service` + `meeting-service` + `transcript-service`) + React Native mobile (`platform-mobile`) + `mfe-meeting` MFE. Faz 23 notify entegre. | Evidence-driven (14-18 hafta MVP) | ADR-0030 KVKK boundary + Observability/Audit GOP skeleton + Gateway Contract 1.0 freeze (cross-repo contract drift riski); GPU yokken CPU PoC önce, model kararı WER ölçüm sonrası | Faz 24 source-side LIVE chain + acceptance D29 (Up/Functional/KVKK-safe) + 3-AI mutabakat (Claude/Codex/Mavis-MiniMax 2026-06-02 — thread `019e879c` AGREE + Mavis msg `78` AGREE) | Faz 22-23 ile paralel; 24.0 charter + 24.1 Gateway Contract → 24.2+ STT sıralı. Cross-repo dependency: PR-gw-01 Contract freeze ÖNCE; PR-stt-02 + PR-stt-03 sonra. | 🟡 24.0 charter started 2026-06-02: live-stt-service PoC iskelet LIVE (platform-ai PR #1 MERGED `4088d9a` — FastAPI + faster-whisper medium int8 + 22/22 test PASS + Codex `019e877b` AGREE); 3-AI mutabakat plan kapısı `019e879c` AGREE final + Mavis `mvs_c922...` msg `78` AGREE (10 mutabakat noktası + 3 RED + revize Faz 24 akışı sabit). Sıradaki: Adım 0 charter PR (ADR-0030 KVKK placeholder + Observability/Audit GOP skeleton + canonical plan docs/faz-24-*); sonra PR-gw-01 Audio Gateway Contract 1.0 freeze (platform-backend Spring WebFlux). Canonical plan: [`docs/faz-24-meeting-intelligence-plan.md`](./docs/faz-24-meeting-intelligence-plan.md) + ADR-0030 (placeholder). |
