...
| D65 | Faz 35 evidence dual-artifact custody | Reporter upload'ı tek mutable object olarak yayınlanmaz. Byte-identical **sealed original** ayrı encrypted/WORM zone'da normal case rollerine kapalı tutulur; günlük vaka erişimi yalnız malware + active-content + metadata kontrollerini geçen, yeniden üretilmiş **sanitized derivative** üzerinden verilir. Upload capability tek object/write-only/short-TTL; extension otorite değildir; archive/container baseline deny; scanner/sanitizer no-network sandbox ve immutable digest/SBOM/provenance ile pinlidir. Standing principal sealed original okuyamaz; original reveal ADR-0047 ile iki farklı insan onayı, conflict/recusal deny, tek kullanım ve en fazla 10 dakika TTL ister. Pipeline yoksa ürün açıkça text-only fail-closed çalışır; report/mailbox kullanılabilir kalır ve attachment sessizce narrative'e gömülmez. Canonical kaynak: [ADR-0048](docs/adr/0048-faz35-evidence-dual-artifact-custody.md) + [`faz35-evidence-custody/v1`](docs/contracts/faz35-evidence-custody.v1.json). Source kararı runtime scanner, production/legal veya gerçek kullanıcı kabulü değildir. |
