...
| D35 | Canlı scoped E2E gate (D29 synthetic'in karşılığı) | **D29 (Zanzibar-ready) `synthetic allow+deny enforce` gerektirir** — bu CI fixture / ephemeral OpenFGA ile karşılanabilir ve Session 30 itibarıyla `openfga-fixture-smoke.yml` + 10 smoke check ile kalıcıdır. Ancak **canlı ürün davranışı ayrı kapıdır**. D35 = staging-sw k3d-test (sonra k3d-prod) üzerinde tam zincir: (a) admin UI'dan scope grant → (b) backend `/api/v1/access/scope` REST → (c) PG `data_access.scope` INSERT (V19 trigger guard fire) → (d) permission-service tuple writer OpenFGA tuple write → (e) `/authz/me` veya business endpoint allow + deny enforce kanıtlı. **D29 ≠ D35**: aynı bar değil. Bu ayrım fixture sonucu canlıymış gibi raporlama riskini engeller (Session 30 Codex 019dcbc8 retrospektif + user 2026-04-26 değerlendirmesi). Detay: `docs/adr/0009-canli-scoped-e2e-gate.md`. |
