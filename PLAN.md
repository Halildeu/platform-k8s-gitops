...
| **22.6** Remote Access Bridge | Agent-initiated outbound remote access / reverse tunnel / session broker hattı. Faz 22.5 command polling yerine geçmez; interaktif ve yüksek yetkili destek oturumları için ayrı güvenlik modeli üretir. | Evidence-driven | Sensitive Endpoint Ops Governance Gate (#1388); OSS-only build-vs-buy matrix (#1400); MeshCentral/RustDesk transport adapter POC (#1401); endpoint-admin broker ADR/state machine (#1402); legacy backend umbrella (#510/#524); agent outbound tunnel spike (#116) | Canonical plan: [`docs/faz-22-remote-access-bridge-plan.md`](./docs/faz-22-remote-access-bridge-plan.md) + **[`ADR-0033`](./docs/adr/0033-faz-22-6-remote-access-bridge-broker.md)** (broker mimarisi + OpenFGA `remote_session` authz + token contract + audit/recording schema + #1388 minimum pilot set + KVKK + STRIDE threat model). **Karar:** broker/policy/audit/recording core bizde yazılır; transport wrap (OpenZiti/zrok + MeshCentral POC); Guacamole yalnız broker-arkası protokol adapter; RustDesk secondary/defer. Inbound port açılmaz; session/device-bound single-use token + dual-control + RBAC + mandatory fail-closed recording + retention olmadan runtime yok. | 22.5 ile paralel planlanabilir; runtime #1388 gate'e bağlı | 🔴 PLANNING / BLOCKED by #1388 (runtime). **Design ACCEPTED via 3-AI consensus 2026-06-09** (Codex `019ea9aa` + Mavis/MiniMax `mvs_c922…` + Claude → ADR-0033); agent-actionable artifacts authored; unblock = #1388 owner/legal decision checklist (plan §9.2) |
...
| **24** Meeting Intelligence / STT Platform | **Bağımsız** toplantı zekâsı ürünü (2026-06-16 owner kararı: **Workcube/ERP bağı YOK** — bağımsız satılır; hedef pazar = Türkiye + regüle/veri-hassas sektörler) — telefon/masaüstü/Teams ses kaynaklarından canlı transkript + konuşmacı ayrımı + özet/karar/aksiyon çıkarımı. STT compute worker (`platform-ai` Python servisleri) + Spring Boot orchestration (`audio-gateway-service` + `meeting-service` + `transcript-service`) + React Native mobile (`platform-mobile`) + `platform-desktop` Electron (Faz 24.13 — board canonical 2026-06-05) + `mfe-meeting` MFE. Faz 23 notify entegre. **Two-server topology (ADR-0031 ACCEPTED, 2026-06-03)**: `platform-ai` ayrı dedicated host'ta; diğer `platform-*` staging-sw'da. | Evidence-driven (14-18 hafta MVP) | ADR-0030 KVKK boundary + ADR-0031 D1-D8 two-server topology + Observability/Audit GOP skeleton + Gateway Contract 1.0 freeze (cross-repo contract drift riski); GPU yokken CPU PoC önce, model kararı WER ölçüm sonrası; cross-server WireGuard + mTLS PKI ZORUNLU | Faz 24 source-side LIVE chain + acceptance D29 (Up/Functional/KVKK-safe/Cross-server transit-safe) + 3-AI mutabakat (Claude/Codex/Mavis-MiniMax 2026-06-02 — thread `019e879c` AGREE + Mavis msg `78` AGREE + Codex `019e8c09` ADR-0031 iter-1+iter-2+iter-3 REVISE absorb 2026-06-03 + Codex `019e97bb`+`019e97c3`+`019e97cc`+`019e97d2` plan canonical sync iter-1/2/3/4 REVISE absorb → iter-5 AGREE 2026-06-05) | Faz 22-23 ile paralel; 24.0 charter + ADR-0031 → 24.1 Gateway Contract → 24.2+ STT sıralı. Cross-repo dependency: ADR-0031 ACCEPTED + cross-server contract field/admission semantics canonical ÖNCE; PR-gw-01 Contract freeze sonra (source-level contract, physical host gerek YOK); platform-ai host provision + k3s ai-test + ArgoCD remote register + Vault AppRole + WireGuard/mTLS + staging-sw Redis Streams runtime setup gerçek meeting audio cross-server e2e / PR-gw-01C öncesi blocker; synthetic/local fixture exception açık. | 🟡 24.0 charter Done 2026-06-03 + 24.1 Gateway 4 PR MERGED + 24.13 Desktop sub-Faz added 2026-06-05: live-stt-service PoC iskelet LIVE (platform-ai PR #1 MERGED `4088d9a` — FastAPI + faster-whisper medium int8 + 22/22 test PASS + Codex `019e877b` AGREE); 3-AI mutabakat plan kapısı `019e879c` AGREE final + Mavis `mvs_c922...` msg `78` AGREE; **ADR-0031 Two-Server Topology ACCEPTED 2026-06-03** (gitops PR #1233 MERGED) — D1 host boundary + D2 network topology + D3 resource pressure gate + D4 GPU + D5 deployment + D6 Vault + D7 KVKK + D8 failure modes; Faz 24 plan §7/§9 + ADR-0030 §"Cross-Server STT Transit Boundary" + Issue #19 re-scope (Faz 24 two-host resource baseline). **PR-gw-01A/B-core/B3 MERGED 2026-06-03** (platform-backend #390+#403+#421): path /api/v1/audio-gateway + Idempotency-Key + AudioSessionRegistry + chunk admission + STREAMING state + dispatcher (DispatchOutcome.QueueFull→429 + Unavailable→503 + Retry-After) + audit sink. **Project #4 board canonical sync 2026-06-05**: 124 item / 15 sub-Faz (24.0-24.X); ghost-Done audit fix + Cancelled option + 24.13 Desktop. **PR-gw-01C MERGED 2026-06-10** (platform-backend#534 Zeynep + iter-2 APPROVE — Redis Streams producer `audio:chunks:p00..p31` + 8-senaryo 429/503) + **PR-stt-04 consumer MERGED** (platform-ai#138 XREADGROUP + messageId dedup). **Aşama-2 staging LIVE 2026-06-11** (platform-ai#151 CLOSED — 6 PR: backend#556/#558 + gitops#1447/#1449/#1450/#1451): audio-gateway k3d-test İLK deploy (D30 digest pin + dual-URI JWT fail-fast + mgmt:8081) + Redis Streams staging-sw host-compose (172.19.0.250, noeviction, runbook `docs/runbooks/redis-streams-staging-sw.md`) + **D29 TAM YEŞİL** (valid-token 201 + chunk 200×2 + XLEN p02=2 + XRANGE PII-guard kanıtı; smoke persona faz24-smoke@acik.com Vault'ta). **2026-06-11 akşam sweep**: ADR-0032 tenant izolasyonu ACCEPTED (platform-ai#65 CLOSED, #152+#153) + ADR-0031 D5 amendment (gitops#1452 — Windows Scheduled Tasks PoC modeli mühürlü, production seçimi açık) + #62 CDC CLOSED (producer-side gate LIVE + 3 repo'ya devir: web#808/mobile#12/desktop#12) + MinIO test profili LIVE (G7 yarısı; health 200 + 3 bucket + audit 7yr lifecycle) + drift gate object-count preflight (gitops#1453). Sıradaki: Aşama-2 koşusu (Zeynep — secret iletimi Halil'de) + G4/G5/G10 hukuk-consent hattı + production deploy modeli kararı (D30 öncesi) + WireGuard/mTLS (gerçek meeting audio öncesi). **2026-06-16 sektör-standardı yol haritası (bağımsız ürün)** canonical plan §11'e eklendi: 5 yetenek hattı (T-A capture / T-B kalite+diarization / T-C intelligence+citation / T-D compliance ürünleştirme / T-E entegrasyon paritesi) + yeni gate'ler (G-WER/G-INT/G-CAP/G-COMP/G-LAT) + Aşama 3-6 sıralı yol + MVP + hibrit GTM; teşhis: altyapı ~%70 olgun, ürün-değer katmanı (capture+intelligence+compliance) eksik (Claude + Codex `019ed1f5` 2-AI sektör istişaresi; Mavis/MiniMax erişilemedi). Canonical plan: [`docs/faz-24-meeting-intelligence-plan.md`](./docs/faz-24-meeting-intelligence-plan.md) §11 + ADR-0030 + ADR-0031 + Project #4 board canonical truth. |
