...
| **24** Meeting Intelligence / STT Platform | Workcube ERP'ye entegre toplantı zekâsı — telefon/masaüstü/Teams ses kaynaklarından canlı transkript + konuşmacı ayrımı + özet/karar/aksiyon çıkarımı. STT compute worker (`platform-ai` Python servisleri) + Spring Boot orchestration (`audio-gateway-service` + `meeting-service` + `transcript-service`) + React Native mobile (`platform-mobile`) + `mfe-meeting` MFE. Faz 23 notify entegre. **Two-server topology (ADR-0031, 2026-06-03)**: `platform-ai` ayrı dedicated host'ta; diğer `platform-*` staging-sw'da. | Evidence-driven (14-18 hafta MVP) | ADR-0030 KVKK boundary + ADR-0031 two-server topology + Observability/Audit GOP skeleton + Gateway Contract 1.0 freeze (cross-repo contract drift riski); GPU yokken CPU PoC önce, model kararı WER ölçüm sonrası; cross-server WireGuard + mTLS PKI ZORUNLU | Faz 24 source-side LIVE chain + acceptance D29 (Up/Functional/KVKK-safe/Cross-server transit-safe) + 3-AI mutabakat (Claude/Codex/Mavis-MiniMax 2026-06-02 — thread `019e879c` AGREE + Mavis msg `78` AGREE + Codex `019e8c09` ADR-0031 iter-1+iter-2+iter-3 REVISE absorb 2026-06-03) | Faz 22-23 ile paralel; 24.0 charter + ADR-0031 → 24.1 Gateway Contract → 24.2+ STT sıralı. Cross-repo dependency: ADR-0031 ACCEPTED + cross-server contract field/admission semantics canonical ÖNCE; PR-gw-01 Contract freeze sonra (source-level contract, physical host gerek YOK); platform-ai host provision + k3s ai-test + ArgoCD remote register + Vault AppRole + WireGuard/mTLS + Redis bounded queue gerçek meeting audio cross-server e2e / PR-queue-01 öncesi blocker; synthetic/local fixture exception açık. | 🟡 24.0 charter started 2026-06-02: live-stt-service PoC iskelet LIVE (platform-ai PR #1 MERGED `4088d9a` — FastAPI + faster-whisper medium int8 + 22/22 test PASS + Codex `019e877b` AGREE); 3-AI mutabakat plan kapısı `019e879c` AGREE final + Mavis `mvs_c922...` msg `78` AGREE; **ADR-0031 Two-Server Topology DRAFT 2026-06-03** — Codex `019e8c09` iter-1+iter-2+iter-3 REVISE absorb (Redis staging-sw, cross-server WireGuard+mTLS, Gate A/B split, k3s ai-test cluster, Vault AppRole `ai-runtime-test`, PR-gw-01 source-level contract); Faz 24 plan §7/§9 + ADR-0030 §"Cross-Server STT Transit Boundary" + Issue #19 re-scope (Faz 24 two-host resource baseline). Sıradaki: Codex iter-4 AGREE → ADR-0031 ACCEPTED → PR-gw-01 source-level contract freeze; platform-ai host provision gerçek cross-server e2e öncesi ops blocker. Canonical plan: [`docs/faz-24-meeting-intelligence-plan.md`](./docs/faz-24-meeting-intelligence-plan.md) + ADR-0030 + ADR-0031. |
